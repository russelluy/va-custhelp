...
RightNow.Widgets.Unsubscribe = RightNow.Widgets.extend({
    ...
    _onResponse: function(response, originalEventObject) {
        ...
        if (originalEventObject.data.action === 'subscribe') {
            action = 'unsubscribe';
            message = this.data.attrs.label_sub_success;
            label = this.data.attrs.label_unsub_button;
        }
        else {
            action = 'subscribe';
            message = this.data.attrs.label_unsub_success;
            label = this.data.attrs.label_resub_button;
        }
        if (button) {
    ...
    _onClick: function(event, type) {
        ...
        if (RightNow.Event.fire(eventName, eo)) {
            RightNow.Ajax.makeRequest(endpoint,
                {filter_type: notificationData.type, id: notificationData.id, cid: this.data.js.contactID},
