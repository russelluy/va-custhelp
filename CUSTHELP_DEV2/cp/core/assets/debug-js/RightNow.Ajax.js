...
RightNow.Ajax = (function(){
    ...
    return {
         ...
         * @param {Object=} requestOptions Configuration options for the request. Valid keys are:
         ...
         *                       ignoreFailure: (boolean) whether to ignore a failed request or to display an error dialog (defaults to false)},
         *      failureHandler: (function) callback function for a failed request,
         *      upload (string), ID of the form that contains files to upload,
         *      timeout: (int) time to wait (in seconds) for the request to complete (defaults to 20000),
         ...
         *      json: (boolean) Whether or not to automatically JSON parse the response before calling successHandler (defaults to false)
         *      isResponseObject: (boolean) Denotes if the request will return a JSON-encoded ResponseObject. If true and the result has an error, a generic
         *                                 error message will be displayed. If the result is valid, only the actual result will be returned. The 'json' option
         *                                 must be enabled for this option to work(defaults to false)
        ...
        makeRequest: function(url, postData, requestOptions)
        {
            ...
            if(!requestOptions.successHandler) {
            ...
            else if(requestOptions.json === true) {
                var userHandler = requestOptions.successHandler,
                    jsonWrapper = function(response, argument, transactionID) {
                        var parsedResponse,
                            callFailureHandler = function(){
                                callback.failureHandler.call(context || window, {"ajaxError": RightNow.Interface.getMessage("ERROR_REQUEST_ACTION_COMPLETED_MSG")}, argument, transactionID);
                            };
                        try {
                            parsedResponse = RightNow.JSON.parse(response.responseText);
                            if(requestOptions.isResponseObject === true){
                ...
                if ("" + userHandler !== "" + jsonWrapper) {
                    // Don't doubly assign this function in the AbuseDetection handler case
