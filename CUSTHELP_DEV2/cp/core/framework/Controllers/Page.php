    ...
    */
    ...
    protected function _renderNonProduction()
    {
        ...
        if ($templateContent)
        {
            ...
            $templateThemes = $this->_parseThemesAndReportErrors($templateContent, $meta['template']);
        ...
        $this->developmentHeader = $this->_createDevelopmentHeaderBuilder($headerDetails);
        ...
        $mainContent = Tags::transformTags(Tags::mergePageAndTemplate($mainContent, $subordinateContent));
    ...
    protected function _isContactAllowed() {
        ...
        return ($this->meta['login_required'] === 'false' || $this->meta['account_session_required'] === 'true' || $this->page === Config::getConfig(CP_LOGIN_URL) || $this->page === Config::getConfig(CP_ACCOUNT_ASSIST_URL));
    }
    ...
    private function _sendContent($content) {
        ...
        if (!headers_sent()) {
            // See this support article http://support.microsoft.com/kb/234067, which suggests sending these headers to prevent caching in IE
            // and also see http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html for the canonical meaning of these headers.
            header('Cache-Control: ' . (Text::stringContains($this->agent->agent_string(), 'MSIE') ? 'max-age=0' : 'no-cache')); // HTTP 1.1 ought to obey this.
            header("Expires: -1"); // HTTP 1.1 ought to obey this.
            header("Pragma: no-cache"); // This is for HTTP 1.0 clients and caches.  The HTTP 1.1 RFC spec suggests this specifically.
     ...
     */
    private function _createDevelopmentHeaderBuilder($headerDetails)
    {
        $meta = $this->_getMetaInformation();
        $javaScriptModule = strtolower($meta['javascript_module']);
        ...
        if (IS_DEVELOPMENT)
        {
            ...
            return new HeaderBuilder\Development($headerDetails, $this->widgetCallsOnPage, $this->page);
        }
        if (IS_REFERENCE)
        {
    ...
    /**
     * Checks if the WAP config is enabled and exists with an error message
     */
    private function _verifyWapNotEnabled()
    {
        // @codingStandardsIgnoreStart
        if (Config::getConfig(wap_interface_enabled))
            exit(Config::getMessage(INTF_DESIG_MOBILE_DEV_MSG));
        // @codingStandardsIgnoreEnd
    }
    ...
    private function _processPostRequest() {
        ...
        if(!Framework::isValidPostToken($this->input->post('validationToken'), $this->input->post('constraints'), ORIGINAL_REQUEST_URI, $postHandler)) {
            PostRequest::addError(Config::getMessage(ERROR_REQUEST_ACTION_COMPLETED_MSG));
            return false;
        }
