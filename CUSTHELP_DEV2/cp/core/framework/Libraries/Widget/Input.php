...
abstract class Input extends Base
{
    ...
    protected function getDataType() {
        ...
        if ($cacheResults = Framework::checkCache($this->cacheKey)) {
            list(
                $errorMessage,
                $cleanName,
                $inputName,
                ...
                $this->dataType,
                $this->data['js']['mask'],
                $this->fieldMetaData) = $cacheResults;
            if($errorMessage) {
                echo $this->reportError($errorMessage);
                return false;
            }
            $this->fieldMetaData = unserialize($this->fieldMetaData);
            $this->data['attrs']['name'] = $cleanName;
        ...
        if(!in_array($this->table, array('Contact', 'Incident', 'Asset'))) {
            return $this->setErrorMessage(sprintf(Config::getMessage(INV_TABLE_PCT_S_EXPECTED_CONT_INC_MSG), $this->table));
        }
        ...
        try {
            list($this->field, $this->fieldMetaData) = Connect::getObjectField($namePieces);
        }
        catch (\Exception $e) {
    ...
    private function setErrorMessage($errorMessage) {
        Framework::setCache($this->cacheKey, array($errorMessage), true);
        echo $this->reportError($errorMessage);
        return false;
    }
    ...
    private function retrieveAndInitializeData() {
        ...
        // Override editability of doing a contact field and PTA or Chat
        if($this->table === 'Contact' && Framework::isLoggedIn() &&
