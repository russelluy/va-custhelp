...
get_instance()->_checkMeta(array('title'=>'' . \RightNow\Utils\Config::msgGetFrom((4453)) . '', 'template'=>'standard.php'));
...
function _standard_notifications_Unsubscribe_view ($data) {
extract($data);
...
?>">
    ...
    <div class="rn_AnswerSummaryDisplay">
        <a href="<?=$this->data['answerUrl'];?>" target='_blank'><?=\RightNow\Utils\Config::msgGetFrom((24466));?></a>
    </div>
    <div id="rn_<?=$this->instanceID;?>_NotificationList">
        <?=$this->data['instructMessage'];?>
        <?if($this->data['js']['notifications']):?>
        <?foreach($this->data['js']['notifications']
as $index => $notification):?>
            <fieldset id="rn_<?=$this->instanceID;?>_<?=$index;?>">
                <legend><?=$notification['label'];?></legend>
                <button data-index="<?=$index;?>"><?=(count($this->data['js']['notifications'])
=== 1 ? $this->data['attrs']['label_resub_button'] : $this->data['attrs']['label_unsub_button']);?></button>
            </fieldset>
        <?endforeach;?>
        <?endif;?>
    </div>
...
$answer = \RightNow\Connect\v1_2\Answer::fetch($answerID);
$this->data['answerUrl'] = '/app/' . \RightNow\Utils\Config::getConfig((222)) . "/a_id/$answerID";
if(!count($notifications) || !$answer) {
$this->data['resultMessage'] = sprintf(\RightNow\Utils\Config::getMessage((47367)), '<b>' . $this->data['email'] . '</b>');
return;
}
$this->data['resultMessage'] = sprintf(\RightNow\Utils\Config::getMessage((47368)), '<b>' . $this->data['email'] . '</b>');
$this->data['answerUrl'] = '/app/' . \RightNow\Utils\Config::getConfig((222)) . "/a_id/$answerID";
...
$this->data['instructMessage'] = \RightNow\Utils\Config::getMessage((47383));
}
else {
$this->data['resultMessage'] = sprintf(\RightNow\Utils\Config::getMessage((1718)), '<b>' . $this->data['email'] . '</b>');
$this->data['instructMessage'] = \RightNow\Utils\Config::getMessage((3131));
}
$notificationDetails = array();
foreach($notifications as $type => $details){
if($type === 'answer'){
$notificationDetails[] = array('type' => $type, 'label' => sprintf("%s - %s", \RightNow\Utils\Config::getMessage((8120)), $answer->Summary), 'id' => intval($answerID));
}
else {
...
$notificationDetails[] = array('type' => $type, 'idChain' => $idChain, 'label' => $labelChain, 'id' => $currentID, 'typeDefine' => ($type === 'product' ? (13) : (14)));
}
}
}
...
return $this->displayError(sprintf(\RightNow\Utils\Config::getMessage((1722)), '<b>' . $this->data['email'] . '</b>'));
}
else {
return $this->displayError();
}
...
echo $message ?: \RightNow\Utils\Config::getMessage((2861));
return false;
}
protected function getProductCategoryChain($notification, $type){
$idChain = $labelChain = '';
$labelChain = ($type === 'product' ? \RightNow\Utils\Config::getMessage((4594)) : \RightNow\Utils\Config::getMessage((4574))) . ' - ';
...
function _standard_notifications_Unsubscribe_header() {
$result = array( 'js_name' => 'RightNow.Widgets.Unsubscribe', 'library_name' => 'Unsubscribe', 'view_func_name' => '_standard_notifications_Unsubscribe_view', 'meta' => array ( 'controller_path' => 'standard/notifications/Unsubscribe', 'view_path' => 'standard/notifications/Unsubscribe', 'js_path' => 'standard/notifications/Unsubscribe', 'presentation_css' => array ( 0 => 'assets/themes/standard/widgetCss/Unsubscribe.css', ), 'base_css' => array ( 0 => 'standard/notifications/Unsubscribe/base.css', ), 'version' => '1.1.1', 'requires' => array ( 'framework' => array ( 0 => '3.2', ), 'jsModule' => array ( 0 => 'standard', 1 => 'mobile', ), ), 'info' => array ( 'description' => 'rn:msg:(4180)', 'urlParameters' => array ( 'email' => array ( 'name' => 'rn:msg:(4547)', 'description' => 'rn:msg:(1724)', ), 'cid' => array ( 'name' => 'rn:msg:(6849)', 'description' => 'rn:msg:(1350)', ), 'a_id' => array ( 'name' => 'rn:msg:(4653)', 'description' => 'rn:msg:(986)', 'example' => 'a_id/3', ), 'unsub' => array ( 'name' => 'rn:msg:(42085)', 'description' => 'rn:msg:(41975)', 'example' => 'unsub/all', ), ), ), 'relativePath' => 'standard/notifications/Unsubscribe', 'widget_name' => 'Unsubscribe', ), );
$result['meta']['attributes'] = array( 'label_unsub_button' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(24417), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(24417), 'inherited' => false, )), 'label_resub_button' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(3423), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(3423), 'inherited' => false, )), 'label_unsub_success' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(24427), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(24427), 'inherited' => false, )), 'label_sub_success' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(24426), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(24426), 'inherited' => false, )), 'delete_notification_ajax' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => '/ci/ajaxRequest/deleteNotification', 'type' => 'ajax', 'default' => '/ci/ajaxRequest/deleteNotification', 'inherited' => false, )), 'add_notification_ajax' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => '/ci/ajaxRequest/addOrRenewNotification', 'type' => 'ajax', 'default' => '/ci/ajaxRequest/addOrRenewNotification', 'inherited' => false, )), );
return $result;
}
}
namespace RightNow\Widgets{
...
.rn_Unsubscribe .rn_Title{padding-bottom:10px;}
.rn_Unsubscribe .rn_AnswerSummaryDisplay{padding:10px;}
.rn_Unsubscribe fieldset{padding-bottom:8px;}
.rn_Unsubscribe button{margin-right:12px;}
.rn_Unsubscribe .rn_AnswerSummaryDisplay{border-bottom:1px dotted #CCC;font-style:italic;margin-bottom:30px;}
.rn_Unsubscribe fieldset{border-bottom:1px solid #CCC;margin:20px 0;}
.rn_Unsubscribe legend{font-weight:bold;margin-bottom:10px;}
.rn_Unsubscribe button{margin-top:6px;}
-->
</style>
...
<div id="rn_PageContent" class="rn_Account">
    <div class="rn_Padding">
        <?=\RightNow\Utils\Widgets::rnWidgetRenderCall('standard/notifications/Unsubscribe',
array());
?>
    </div>
</div>
...
( 0 => 'standard/notifications/Unsubscribe', ), '/account/notif/unsubscribe.js', '1441160703');?>"></script>
