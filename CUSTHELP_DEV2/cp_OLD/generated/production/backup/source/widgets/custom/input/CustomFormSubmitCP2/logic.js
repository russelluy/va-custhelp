...
RightNow.Widget.CustomFormSubmit = function(data, instanceID) {
...
RightNow.Widget.CustomFormSubmit.prototype = {
    /**
    ...
    _onFormValidationFail: function()
    {
        if (RightNow.UI.Form.form === this._parentForm)
        {
            var Dom = YAHOO.util.Dom,
                div = this._errorMessageDiv;
            //give error div a common error message CSS class
            Dom.addClass(div, "rn_MessageBox");
            Dom.addClass(div, "rn_ErrorMessage");
            Dom.removeClass(div, "rn_Hidden");
            if(div.tabIndex === 0)
            {
                window.scrollTo(0, Dom.getY(div) - 20); //20px buffer above error div
                //focusing half a second later helps screen readers announce the message correctly
                YAHOO.lang.later(500, this, function() { div.tabIndex = 0; div.focus(); div.tabIndex = 1; });
            }
            else
            {
                //focus first link in the error box and scroll to it
                var firstField = Dom.getElementBy(function(e){return true;}, "A", div);
