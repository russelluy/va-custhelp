...
RightNow.Field = RightNow.Field || RightNow.EventProvider.extend(/**@lends RightNow.Field*/{
        ...
        if(this._value !== null && (this.data.js.constraints.maxLength || this.data.js.constraints.minLength)) {
            ...
            if(maxLength && maxLength < length) {
                return this._reportError(RightNow.Text.sprintf((length - maxLength === 1)
                    ...
                    : RightNow.Interface.getMessage("EXCEEDS_SZ_LIMIT_PCT_D_CHARS_PCT_D_LBL"),
    ...
    */
    ...
    _initializeHint: function() {
        ...
        {
            if(this.Y.Overlay) {
                var overlay = this.Y.Node.create("<span id='" + this.baseDomID + "_Hint' class='rn_HintBox'>" + this.data.attrs.hint + "</span>"),
                    node = (this.input instanceof this.Y.NodeList) ? this.input.item(this.input.size() - 1) : this.input;
                while (node.next())
                    node = node.next();
                ...
                this.input.get("parentNode").append(this.Y.Node.create("<span class='rn_HintText'>" + this.data.attrs.hint + "</span>"));
            }
        }
    }
}, {
