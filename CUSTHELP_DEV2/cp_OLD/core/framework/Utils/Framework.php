...
final class Framework extends \RightNow\Internal\Utils\Framework
{
    ...
    public static function createPostToken($constraints, $action, $handler) {
        ...
        return Api::encode_base64_urlsafe(Api::pw_rev_encrypt(implode('|', array(sha1($constraints), Api::intf_name(), $action, $handler))));
    }
    ...
    public static function isValidPostToken($token, $constraints, $action, $handler) {
        if(!$decryptedToken = Api::pw_rev_decrypt(Api::decode_base64_urlsafe($token))) {
            return false;
        }
        $action = Url::deleteParameter($action, 'session');
     ...
     * @return void
     */
    public static function runSqlMailCommitHook()
    {
     ...
     *  header.
     */
    public static function setLocationHeader($headerValue) {
