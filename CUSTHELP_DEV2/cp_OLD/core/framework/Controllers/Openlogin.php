...
final class OpenLogin extends Base{
    ...
    public function provideEmail(){
        ...
        if($email && $encryptedUserData){
            ...
            if($userData->id && $userData->providerName && $userData->userName && $userData->firstName && $userData->lastName){
                $user = new ThirdPartyUser($userData->providerName, null);
                $user->id = $userData->id;
    ...
    private function _validateRedirect($redirectUrl = null) {
        ...
        $getParsed = function($url) {
            if ($parsed = @parse_url($url)) {
                ...
                $parsed['isRelative'] = (!$parsed['host'] && !$parsed['scheme'] && $parsed['dirname'] !== '.' && $parsed['dirname'] !== '/');
                return $parsed;
            }
        };
    ...
    private function _authorizeOpenID($providerUrl, $redirectUrl){
        ...
        if($openIDUrl){
            ...
            try{
                $this->client->redirectToThirdPartyLogin($openID->authUrl());
            }
    ...
    private function _callbackFacebook(array $redirectSegments){
        ...
        if($requestToken){
            ...
            $accessToken = $this->client->post(self::FB_ACCESS_URL, array(
                'code' => $requestToken,
                ...
                'redirect_uri' => sprintf(Url::deleteParameter($this->oAuthCallbackUrl, 'session'), 'facebook', $redirectUrl),
    ...
    */
     ...
     * * screen_name: string
     *
     * @return ThirdPartyUser Object containing the user's profile info
     */
    private function _getTwitterUserInfo(array $oAuthResponse){
        ...
        return new ThirdPartyUser('twitter', $userInfo);
    }
    ...
    private function _getFacebookUserInfo($accessToken){
        ...
        $userInfo = json_decode($this->client->get(self::FB_CONTACT_API_URL . '?' . $accessToken));
        return new ThirdPartyUser('facebook', $userInfo);
    }

    /**
     * Retrieves the user's info from Google's API.
     * @param  string $accessToken A valid Google access token
     * @return object Object ThirdPartUser containing the user's profile info and email
     */
    private function _getGoogleUserInfo($accessToken) {
        ...
        return new ThirdPartyUser('google', $userInfo);
    }
    ...
    private function _getOpenIDUserInfo($openIDObject, $openIDUrl){
        return new ThirdPartyUser('openid', $openIDObject->getAttributes() + array('openIDUrl' => $openIDUrl));
    }
     ...
     * @param array $parameters Contains
     *   $provider,
     ...
     */
    private function _dealWithFacebook(array $parameters){
        $action = $parameters[0];
     ...
     * Returns
     * @param ThirdPartyUser $user ThirdPartyUser
     ...
     */
    private function _loginUser(ThirdPartyUser $user, $providerName){
        $contact = null;
        ...
        if ($contact && Config::getConfig(COMMUNITY_ENABLED, 'RNW')) {
        ...
        if ($contact) {
            return $this->_doLogin($contact, $providerName, $user->id);
        }
     ...
     *       contact.login has been cleared out since the user last logged-in
     * Logs a valid user in.
     * @param ThirdPartyUser $user Must contain firstName, lastName, email members.
     ...
     *         String Encrypted serialized user object if user data is provided but an email isn't provided.
     */
    private function _logInOpenIDUser(ThirdPartyUser $user, $openIDUrl){
     ...
     * @param ThirdPartyUser $user ThirdPartyUser object containing firstName, lastName properties
     */
    private function _handleCommunityIntegration($contactID, $newContact, ThirdPartyUser $user) {
     ...
     * @param ThirdPartyUser $user ThirdPartyUser object
     ...
     */
    private function _handleInvalidEmail(ThirdPartyUser $user, $providerName) {
     ...
     * @param ThirdPartyUser $user ThirdPartyUser data provided by a service
     ...
     */
    private function _updateContactFields(Connect\Contact $contact, ThirdPartyUser $user, $providerName){
        $toUpdate = array();
        ...
        if($providerName !== 'openid'){
            $toUpdate += $user->getChannelArray();
        }
        else{
            $openIDAccountFound = false;

            // if OpenID accounts already exist, verify that it doesn't already exist
            ...
            if(!$openIDAccountFound){
                $toUpdate['Contact.OpenIDAccounts.0.URL'] = (object) array('value' => $user->openIDUrl);
            }
        }
        return $toUpdate;
    }
     ...
     * @param string $userID The user's id on the third-party service
     * @param string $providerDomain Domain name of the provider being used. Only provided when logging in via OpenID
     * @return mixed Bool true if the operation was successful or an Int error code if the contact is disabled.
     */
    private function _doLogin(Connect\Contact $contact, $provider, $userID = null, $providerDomain = null){
    ...
    private function _getRequestingPage(){
        ...
        if(Text::stringContains($this->referrer, $cpBaseUrl)){
            ...
            return rtrim(Url::deleteParameter(Url::deleteParameter(Url::deleteParameter(Text::getSubStringAfter($this->referrer, $cpBaseUrl), 'redirect'), 'oautherror'), 'emailerror'), '/');
        }
...
class Client {
    ...
    function redirectBackToCPPage($pageUrl = null){
        ...
        if (IS_PRODUCTION) {
            ...
            \RightNow\Utils\Framework::writeContentWithLengthAndExit('Host not allowed, check CP_REDIRECT_HOSTS config.');
        }
    }
...
/**
 * Object used to store third party data prior to inserting it into the DB
 *
 * @internal
 */
class ThirdPartyUser {
    ...
    /**
     * Set when the necessary data isn't supplied
     */
    public $error = false;

    /**
     * Used for OpenID + OAuth
     */
    private $firstName;
    private $lastName;
    public $email;
    ...
    /**
     * Constructor.
     * @param string $type Either twitter, facebook, or openid
     * @param array|object $userInfo JSON-decoded data from the third-party
     */
    function __construct($type, $userInfo) {
        $this->type = $type;
        $method = "{$this->type}User";
    ...
    /**
     * Setter for first and last names. Truncates name fields if values are longer than what's acceptable.
     * @param string $name Property name
     * @param string $value Property value
     * @return string The newly-set value
     */
    function __set($name, $value) {
        if (($name === 'firstName' || $name === 'lastName') && Text::getMultibyteStringLength($value) > OpenLogin::MAX_CONTACT_NAME_LENGTH) {
            $value = Text::truncateText($value, OpenLogin::MAX_CONTACT_NAME_LENGTH, false);
        }
        return $this->{$name} = $value;
    }

    /**
     * Getter for first and last names. Returns the specified property's value.
     * @param string $name Property name
     * @return string value
     */
    function __get($name) {
    ...
    /**
     * Sets data received from Twitter.
     * @param object $userInfo JSON-decoded data from Twitter
     */
    private function twitterUser($userInfo) {
        if (!$userInfo || !$userInfo->id_str) {
            $this->error = true;
            return;
        }

        $this->avatarUrl = $userInfo->profile_image_url;
        list($firstName, $lastName) = explode(' ', $userInfo->name, 2);
        $this->__set('firstName', $firstName);
        $this->__set('lastName', $lastName ?: $firstName);
        $this->userName = $userInfo->screen_name;
        $this->id = $userInfo->id_str;
    }

    /**
     * Sets data received from Facebook.
     * @param object $userInfo JSON-decoded data from Facebook
     */
    private function facebookUser($userInfo) {
        if (!$userInfo || !$userInfo->id) {
            $this->error = true;
            return;
        }

        $this->avatarUrl = sprintf(OpenLogin::FB_PROFILE_PIC_URL, $userInfo->id);
        $this->__set('firstName', $userInfo->first_name);
        $this->__set('lastName', $userInfo->last_name);
    ...
    /**
     * Sets data received from Google.
     * @param  object $userInfo JSON-decoded data from Google
     */
    private function googleUser($userInfo) {
        if (!$userInfo || !$userInfo->email) {
            $this->error = true;
            return;
        }

        $this->type = 'openid'; // Google accounts don't have a channel mapping; we'll fall back to openid.
        $this->email = $userInfo->email;
        $this->openIDUrl = $userInfo->profile;
        $this->avatarUrl = $userInfo->picture;
        $this->__set('firstName', $userInfo->given_name);
        $this->__set('lastName', $userInfo->family_name);
        $this->id = $userInfo->sub;
    }

    /**
     * Sets data received from LightOpenID.
     * @param array|null $userInfo OpenID fields
     */
    private function openidUser($userInfo) {
        if (!$userInfo || !$userInfo['contact/email']) {
            $this->error = true;
            return;
        }
        ...
        $this->__set('firstName', $userInfo['namePerson/first'] ?: '');
        $this->__set('lastName', $userInfo['namePerson/last'] ?: '');
        $this->openIDUrl = $userInfo['openIDUrl'];
        $this->email = $userInfo['contact/email'];
    }

    /**
     * Returns an array to be accepted for Contact model creates / updates.
     * @return array Contact data
     */
    function toContactArray() {
        $return = array();

        foreach ($this->contactConversion as $contactField => $userField) {
            $return[$contactField] = (object) array('value' => $this->{$userField});
        }

        if ($this->type === 'openid') {
            $return += array('Contact.OpenIDAccounts.0.URL' => (object) array('value' => $this->openIDUrl));
        }
        else {
    ...
    /**
     * Returns a ChannelUsernames array to be accepted for Contact model creates / updates.
     * @return array ChannelUsernames data
     */
    function getChannelArray() {
        $fieldName = 'Contact.ChannelUsernames.' . strtoupper($this->type);
        return array(
            "$fieldName.Username" => (object) array('value' => $this->userName ?: $this->email),
            "$fieldName.UserNumber" => (object) array('value' => $this->id),
        );
    }
}
