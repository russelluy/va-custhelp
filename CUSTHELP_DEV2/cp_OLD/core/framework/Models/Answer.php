...
class Answer extends Base
{
    ...
    public function getRelatedAnswers($answerID, $limit)
    {
        ...
        try{
            ...
            if ($relatedAnswers) {
                // Escape HTML/Script for results in title
                foreach($relatedAnswers as $answer)
                    $answer->Title = Text::escapeHtml($answer->Title);
            }
        }
        catch(Connect\ConnectAPIErrorBase $e){
            return $this->getResponseObject(null, null, $e->getMessage());
    ...
    public function getPreviousAnswers($answerID, $limit, $truncateSize) {
        ...
        if (!$sessionUrlParams || !($answerIDs = $sessionUrlParams['a_id'])) {
            return $response;
        }
        else {
        ...
        try{
            $queryResult = Connect\ROQL::query('SELECT A.ID, A.Summary, A.AccessLevels FROM Answer A WHERE A.ID IN (' . implode(',', $answers) . ') AND A.StatusWithType.Status.ID = ' . STATUS_TYPE_PUBLIC)->next();
        }
        catch(Connect\ConnectAPIErrorBase $e){
        ...
        $contactAccess = 0;
        foreach(explode(',', Api::contact_answer_access()) as $id) {
            $contactAccess += pow(2, $id - 1);
        }

        $previousAnswers = array();
        while($row = $queryResult->next()){
            if(!(intval($row['AccessLevels']) & $contactAccess))
                continue;
     ...
     */
    public function getAnswerSummary($answer, $escapeHtml = true)
    {
        ...
        try{
            $queryResult = Connect\ROQL::query("SELECT A.ID, A.Summary, A.Language.ID as LanguageID, A.StatusWithType.StatusType.ID as StatusType, A.AccessLevels FROM Answer A WHERE A.ID $whereClause")->next();
        }
        catch(Connect\ConnectAPIErrorBase $e){
            return $this->getResponseObject(null, null, $e->getMessage());
        }
        ...
        $results = array();
        while($row = $queryResult->next()){
            //Skip this Answer if the contact doesn't have the appropriate access level
            if(!(intval($row['AccessLevels']) & $contactAccess))
                continue;
    ...
    public function emailToFriend($sendTo, $name, $from, $answerID) {
        ...
        if(($answer = $this->getAnswerSummary($answerID)->result)) {
