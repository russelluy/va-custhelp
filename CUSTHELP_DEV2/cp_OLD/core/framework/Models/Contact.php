...
class Contact extends PrimaryObjectBase
{
    ...
    public function get($contactID = null){
        if($contactID === null){
            if(Framework::isLoggedIn()){
                ...
                return $this->getResponseObject(null, null, "No contact ID specified and no contact is logged in.");
            }
        }
    ...
    public function getChannelFields($loginOrID) {
        ...
        try {
            $query = Connect\ROQL::query("SELECT ChannelUsernames.Channel.ID, ChannelUsernames.UserNumber, ChannelUsernames.Username
                                                  FROM Contact
    ...
    public function doLogin($username, $password, $sessionID, $widgetID, $url)
    {
        ...
        {
            // Failed to login
            $result['message'] = (Config::getConfig(CP_MAX_LOGINS))
    ...
    public function getProfileSid($username, $password, $sessionID)
    {
        ...
        $profile = $this->CI->session->createMapping(Api::contact_login($pairData));
    ...
    public function lookupContactByOpenLoginAccount($providerName, $openLoginAccountID, $openLoginAccountUsername = '') {
        ...
        if ($providerName === 'openid') {
            ...
            $query = "SELECT Contact FROM Contact C WHERE C.ChannelUsernames.Channel.ID = $providerID AND (C.ChannelUsernames.UserNumber = $openLoginAccountID $optionalCriteria)";
        }
    ...
    public function sendLoginEmail($email)
    {
        ...
        $result = $this->getResponseObject(array('message' => '<b>' . Config::getMessage(EMAIL_CONTAINING_ACCT_INFORMATION_MSG) . '</b><p></p>' . Config::getMessage(DNT_RECEIVE_EMAIL_UL_THAN_LI_ACCT_MSG)), 'is_array');
    ...
    public function sendResetPasswordEmail($login) {
        $result = $this->getResponseObject(array('message' => '<b>' . Config::getMessage(EMAIL_RESET_PASSWORD_MSG) . '</b><p></p>' . Config::getMessage(DONT_RECEIVE_EMAIL_UL_THAN_LI_ACCT_MSG)), 'is_array');
    ...
    */
    ...
    protected function updateProfile(Connect\Contact $contact, $newPassword) {
        ...
        $oldProfile = $this->CI->session->getProfile(true);
        if ($oldProfile->openLoginUsed) {
            $newProfile->openLoginUsed = $oldProfile->openLoginUsed;
        }
        $newProfile = $this->CI->session->createMapping($newProfile);
