...
class Incident extends PrimaryObjectBase {
    ...
    public function create(array $formData, $smartAssist = false) {
        ...
        foreach ($formData as $name => $field) {
            ...
            if($smartAssist === true && $field->value !== null){
                ...
                if(in_array($fieldMetaData->COM_type, array('NamedIDLabel', 'ServiceProduct', 'ServiceCategory', 'Asset'))){
                    $name .= ".ID";
                }
    ...
    public function submitFeedback($answerID, $rate, $threshold, $name, $message, $givenEmail, $numberOfOptions = null) {
        if($givenEmail !== null && !Text::isValidEmailAddress($givenEmail)) {
    ...
    protected function convertFormDataToSmartAssistantSearch(array $formData, Connect\Contact $contact){
        ...
        foreach($formData as $fieldName => $fieldValue){
            ...
            if(Text::beginsWith($fieldName, 'Incident.CustomFields') || in_array($fieldName, array('Incident.Disposition', 'Incident.Language', 'Incident.Queue', 'Incident.Severity', 'Incident.StatusWithType'))){
