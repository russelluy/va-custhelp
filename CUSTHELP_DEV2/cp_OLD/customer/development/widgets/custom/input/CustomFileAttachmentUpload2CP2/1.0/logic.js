RightNow.namespace('Custom.Widgets.input.CustomFileAttachmentUpload2');
Custom.Widgets.input.CustomFileAttachmentUpload2 = RightNow.Widgets.extend({ 
    /**
     * Widget constructor.
     */
    constructor: function() {
     ...
     Y.Event.addListener(this._inputField, "change", this._onFileAdded, null, this);
     Y.Event.addListener(this._inputField, "keypress", this._onKeyPress, null, this);
     //pasting === bad
     Y.Event.addListener(this._inputField, "paste", function(){return false;});
     ...
     */
     ...
     _onKeyPress: function(event)
     {
         ...
         else if(Y.UA.ie && keyPressed === Key.ENTER)
         {
     ...
     _onFileAdded: function()
     {
         ...
         {
             myNode.removeClass(this._inputField.id, "rn_ErrorField");
             ...
             if(!allowSubmit)
             {
                 myNode.addClass(this._inputField.id, "rn_ErrorField");
                 myNode.addClass("rn_" + this.instanceID + "_Label", "rn_ErrorLabel");
                 if(this._statusMessage)
                 {
                     myNode.removeClass(this._statusMessage, "rn_ScreenReaderOnly");
     ...
     _fileUploadReturn: function(type, response)
     {
         if (response[1].instanceID === this.instanceID)
         {
             ...
             if(this._statusMessage)
             {
                 myNode.addClass(this._statusMessage, "rn_ScreenReaderOnly");
             ...
             // Chrome and Safari are both truthy on Y.UA.webkit
             ...
             if(!this._attachmentList)
                 this._attachmentList = afterNode.insert(document.createElement("ul"), this._statusMessage);
