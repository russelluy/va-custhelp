...
class Prodcat extends Base {
     ...
     * @param int $level Hierarchy level. A level of 1 will return first level products/categories (i.e. those not having a parent).
     * @param int $limit Max number of top level product or categories to return. This limit applies to each level so if the limit is set to 5 and $level
     *                   is set to 2, for example, you'd get 5 top level items, each of which would have a max of 5 sub items returned.
     ...
     */
    public function getHierarchy($filterType, $level, $limit = 30, array $topLevelIDs = array()) {
        ...
        if (!$hierarchy = $this->getCached($cacheKey)) {
            try {
                ...
                $children = $level > 1 ? $this->getDirectDescendantsFromList($parents, $connectName, $limit) : array();
    ...
    */
    ...
    public function getHierPopup($filterType, $linkingValue = null) {
        ...
        if ($linkingValue) {
            if (!Framework::isValidID($linkingValue) || (!$categoryIDs = $this->getChildCategories($linkingValue))) {
                $response->warning = \RightNow\Utils\Config::getMessage(LINKING_VAL_LINKINGVALUE_0_LINKED_LBL);
                return $response;
            }
    ...
    public function getChain($id, $level, $ignoreVisibility = false) {
        ...
        if (!Framework::isValidID($level) || !in_array($level, range($minLevel, $maxLevel))) {
            $response->error = \RightNow\Utils\Config::getMessage(LEVEL_INTEGER_MINLEVEL_MAXLEVEL_MSG);
            return $response;
        }

        if (!$ignoreVisibility && !$this->isEnduserVisible($connectObject)) {
            $response->warning = \RightNow\Utils\Config::getMessage(RETURNING_EMPTY_ARRAY_ID_ENDUSER_MSG);
