...
class AbuseDetection
{
    ...
    public static function isAbuse($formToken=null) {
        if (self::$isAbuse === null || IS_UNITTEST) {
            ...
            if (!self::$isAbuse) {
                \RightNow\Connect\v1_2\CustomerPortal::unlockAbusableMethods();
            }
        }
        return self::$isAbuse;
    }
    ...
    public static function check($formToken=null) {
        if (self::isAbuse($formToken)) {
            ...
            $json = array(
                ...
                'dialogCaption' => $isRetry ? Config::getMessage(RESPONSE_ACCEPTED_PLEASE_TRY_MSG) : Config::getMessage(PLS_VERIFY_REQ_ENTERING_TEXT_IMG_MSG),
            );
    ...
    public static function getChallengeProvider() {
        $languageCode = substr(LANG_DIR, 0, 2);
        if (!in_array($languageCode, array("en", "nl", "fr", "de", "pt", "ru", "es", "tr"), true)) {
            $languageCode  = "en";
        }
        $scriptUrl = "//www.google.com/recaptcha/api/js/recaptcha_ajax.js";
        $providerScript = 'var defaultChallengeOptions = {
            scriptUrl: "' . Text::escapeStringForJavaScript($scriptUrl) . '",
            publicKey: "' . Text::escapeStringForJavaScript(Config::getConfig(ADS_CHALLENGE_PUBLIC_KEY)) . '",
            lang: "' . $languageCode . '",
            custom_translations: {
