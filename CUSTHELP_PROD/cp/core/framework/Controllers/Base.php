...
class Base extends \Controller
{
    ...
    public function __construct($loadRnow = true)
    {
        ...
        if ($mode && $mode !== 'test' && (!$modeToken || !Framework::testCsrfToken($modeToken, crc32($mode))))
        {
    ...
    protected function _setAgentSessionIdCookie($sessionID) {
        ...
        if ($sessionID !== $this->_getAgentSessionIdFromCookie()) {
            ...
            setcookie(self::agentSessionIdKey, $sessionID, 0, '/');
        }
    }
    ...
    {
        return $this->_getViewsPath() . ((IS_OPTIMIZED) ? 'headers/' : 'pages/');
    }
