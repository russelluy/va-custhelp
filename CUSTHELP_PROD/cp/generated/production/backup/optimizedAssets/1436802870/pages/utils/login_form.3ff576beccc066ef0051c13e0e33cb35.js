...
RightNow.Widgets.LoginForm=RightNow.Widgets.extend({constructor:function(){this.Y.one(this.baseSelector+"_Submit").on("click",this._onSubmit,this);this._usernameField=this.Y.one(this.baseSelector+"_Username");this._passwordField=this.Y.one(this.baseSelector+"_Password");if(this.data.attrs.initial_focus)
{if(this._usernameField&&this._usernameField.get("value")==='')
this._usernameField.focus();else if(this._passwordField)
this._passwordField.focus();}},_getRedirectUrl:function(result){var redirectUrl;if(this.data.js&&this.data.js.redirectOverride)
...
eventObject=new RightNow.Event.EventObject(this,{data:{login:username,password:((!this.data.attrs.disable_password&&this._passwordField)?this._passwordField.get("value"):""),url:window.location.pathname,w_id:this.data.info.w_id}});if(RightNow.Event.fire("evt_loginFormSubmitRequest",eventObject)){this._toggleLoading(true);if(RightNow.Event.noSessionCookies()){RightNow.Event.setTestLoginCookie();}
