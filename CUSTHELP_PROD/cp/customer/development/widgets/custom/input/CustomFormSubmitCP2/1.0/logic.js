...
Custom.Widgets.input.CustomFormSubmit = RightNow.Widgets.extend({ 
    /**
     * Widget constructor.
     */
    constructor: function() {
	    ...
	    if(!this._errorMessageDiv)
	    {
	        ...
	        this._errorMessageDiv = beforeNode.insert(errorNode, this._formButton);
	    }
	    this._errorMessageDiv.tabIndex = -1;
	    myNode.setAttrs(this._errorMessageDiv, "aria-live", "rude");
    ...
    _onButtonClick: function(type, args)
    {
        ...
        myNode.addClass(this._errorMessageDiv, "rn_Hidden");
        this._errorMessageDiv.innerHTML = "";

        var eo = new RightNow.Event.EventObject();
        eo.w_id = this.instanceID;
        eo.data = {"form" : this._parentForm, "error_location" : this._errorMessageDiv.id, "f_tok" : this.data.js.f_tok};
        ...
        if(Y.UA.ie !== 0 && window.external && "AutoCompleteSaveForm" in window.external)
    ...
    _onFormValidated: function()
    {
        if(RightNow.UI.Form.form === this._parentForm && RightNow.UI.Form.formFields.length > 0)
        {
            ...
            myNode.removeClass("rn_" + this.instanceID + "_LoadingIcon", "rn_Hidden");
    ...
    _onFormValidationFail: function()
    {
        ...
        {
            var div = this._errorMessageDiv;
            //give error div a common error message CSS class
            myNode.addClass(div, "rn_MessageBox");
            myNode.addClass(div, "rn_ErrorMessage");
            myNode.removeClass(div, "rn_Hidden");
            if(div.tabIndex === 0)
            {
                window.scrollTo(0, myNode.getY(div) - 20); //20px buffer above error div
                //focusing half a second later helps screen readers announce the message correctly
                Y.lang.later(500, this, function() { div.tabIndex = 0; div.focus(); div.tabIndex = 1; });
            }
            else
            {
                //focus first link in the error box and scroll to it
                var firstField = myNode.all(function(e){return true;}, "A", div);
    ...
    _createChallengeDiv: function()
    {
        ...
        if (!challengeDiv) {
            ...
            beforeNode.insert(challengeDiv, this._formButton);
        }
    },
    ...
    */
    _enableFormExpirationWatch: function()
    {
        if(this.data.js.formExpiration >= 300000)
        {
            ...
            {
                Y.lang.later(this.data.js.formExpiration, this, function(){
                    RightNow.UI.Dialog.messageDialog(RightNow.Interface.getMessage("FORM_EXP_PLS_CONFIRM_WISH_CONTINUE_MSG"), {icon: "WARN", exitCallback: {fn: function(){
                        RightNow.Ajax.makeRequest("/ci/ajaxRequest/getNewFormToken", {formToken: this.data.js.f_tok}, {
    ...
    {
        this._enableClickListener();
        myNode.addClass("rn_" + this.instanceID + "_LoadingIcon",  "rn_Hidden");
        if(this._statusMessage)
            this._statusMessage.innerHTML = "";
    },
    ...
    _enableClickListener: function()
    {
        ...
        Y.Event.addListener(this._formButton, "click", this._onButtonClick, null, this);
    },
    ...
    _disableClickListener: function()
    {
        ...
        Y.Event.removeListener(this._formButton, "click", this._onButtonClick);
    }
