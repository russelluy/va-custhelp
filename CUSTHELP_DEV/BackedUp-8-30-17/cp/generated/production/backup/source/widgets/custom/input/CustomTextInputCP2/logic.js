...
RightNow.Widget.CustomTextInput = function(data, instanceID) {
    this.data = data;
...
*/
    ...
    if(this.data.attrs.validate_on_blur)
    {
        ...
        if(this.data.attrs.require_validation)
            YAHOO.util.Event.addListener(this._validationField, "blur", this._blurValidateValidationField, null, this);
    }
...
RightNow.Widget.CustomTextInput.prototype = {
/**
 * ----------------------------------------------
...
*/
    ...
    _onValidate: function(type, args)
    {
        ...
        if (RightNow.UI.Form.form === this._parentForm)
        {
            ...
            if(this._compareInputToMask(true) && this._checkRequired() && this._checkData() && this._checkValue() && this._checkEmail() && this._checkUrl() && this._checkRequiredValidationField() && this._checkValidationField())
            {
                ...
                if(this.data.attrs.require_validation)
                {
                    YAHOO.util.Dom.removeClass(this._validationField, "rn_ErrorField");
                    YAHOO.util.Dom.removeClass("rn_" + this.instanceID + "_LabelValidate", "rn_ErrorLabel");
                }
    ...
    */
    ...
    {
        if(this.data.attrs.require_validation && YAHOO.lang.trim(this._validationField.value) !== this._inputField.value)
        {
            var validationLabel = RightNow.Text.sprintf(this.data.attrs.label_validation, this._errorLabel);
    ...
    */
    ...
    _displayValidationError: function(errorMessage)
    {
        if(this.data.attrs.require_validation)
        {
            ...
            YAHOO.util.Dom.addClass(this._validationField.id, "rn_ErrorField");
            YAHOO.util.Dom.addClass("rn_" + this.instanceID + "_LabelValidate", "rn_ErrorLabel");
        }
    },
    ...
    _errorHandler: function(errorMessage, inputLabel, labelID, fieldID)
    {
        ...
        YAHOO.util.Dom.addClass(fieldID, "rn_ErrorField");
        YAHOO.util.Dom.addClass(labelID, "rn_ErrorLabel");
    },
 ...
 */
    ...
    _initializePasswordOverlay: function() {
        ...
        Event.addListener(element, "keydown", function(e, type) {
            ...
            if (e.keyCode === YAHOO.util.KeyListener.KEY.TAB) {
    ...
    _showPasswordOverlay: function(e, type) {
        ...
        if (!this[name]) {
            var Dom = YAHOO.util.Dom,
                content = document.getElementById('rn_' + this.instanceID + '_PasswordOverlay'),
                overlay = document.createElement("span"),
                removeClass = (type === 'input') ? 'rn_Hidden' : 'rn_ScreenReaderOnly';
            
            Dom.removeClass(content, removeClass);
            Dom.insertAfter(overlay, this._inputField);
            ...
            if (YAHOO.widget.Overlay) {
                ...
                overlay = {
                    body: overlay,
                    hide: function() { Dom.addClass(this.body, 'rn_Hidden'); },
                    show: function() { Dom.removeClass(this.body, 'rn_Hidden'); },
                    cfg: {
                        setProperty: function(prop, val) {
                            if (prop === 'zIndex') {
                                Dom.setStyle(this.body, prop, val);
                            }
                        },
                        getProperty: function(prop) {
                            if (prop === 'visible') {
                                return !Dom.hasClass(this.body, 'rn_Hidden');
                            }
                        }
                    }
                };
    ...
    _passwordBlurValidation: function(e, type) {
        ...
        if (this["_validatePassword" + type.charAt(0).toUpperCase() + type.slice(1)](true)) {
            ...
            if (e.keyCode === YAHOO.util.KeyListener.KEY.TAB && !e.shiftKey && overlay.cfg.getProperty('visible')) {
                // Stop the tab event from skipping
                // the validation overlay's focus
                YAHOO.util.Event.preventDefault(e);
            }
        }
        YAHOO.util.Dom[classMethod]('rn_' + this.instanceID + '_Label', 'rn_ErrorLabel');
        YAHOO.util.Dom[classMethod](this._inputField, "rn_ErrorField");
    },
    ...
    */
    ...
    _validatePasswordInput: function(e) {
        if (e.keyCode === YAHOO.util.KeyListener.KEY.TAB) return;
        ...
        YAHOO.util.Dom.addClass(YAHOO.util.Dom.getElementsByClassName('rn_Title', 'div', 'rn_' + this.instanceID + '_PasswordOverlay'), 'rn_Hidden');
        var container = YAHOO.util.Dom.getElementBy(function(node) {
                return YAHOO.util.Dom.hasClass(node, 'rn_Strength');
            }, 'div', 'rn_' + this.instanceID + '_PasswordOverlay'),
            meter = YAHOO.util.Dom.getElementsByClassName('rn_Meter', 'div', container);
        YAHOO.util.Dom.removeClass(container, 'rn_Hidden');
        if (meter[0]) {
    ...
    _updatePasswordChecklist: function(name, action) {
        ...
        YAHOO.util.Dom.getElementBy(function(node) {
