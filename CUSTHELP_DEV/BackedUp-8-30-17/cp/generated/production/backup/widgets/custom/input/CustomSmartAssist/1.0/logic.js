RightNow.namespace('Custom.Widgets.input.CustomSmartAssist');
Custom.Widgets.input.CustomSmartAssist = RightNow.Field.extend({ 
    overrides: {
    ...
    _displayResults: function(evt, args)
    {
        ...
        if(result && result.data && result.data.result && result.data.result.sa)
        {
            ...
            if(suggestions && suggestions.length > 0)
            {
                ...
                saDisplay = new EJS({text: this.getStatic().templates.customDisplayResults}).render({
        ...
        if(expand)
        {
            for(var i in this._answersLoaded)
            {
                if(this._answersLoaded.hasOwnProperty(i) && i != answerID && this._answersLoaded[i] === true)
                {
                    ...
                    this.Y.one(id + i).set("aria-expanded", "false");
