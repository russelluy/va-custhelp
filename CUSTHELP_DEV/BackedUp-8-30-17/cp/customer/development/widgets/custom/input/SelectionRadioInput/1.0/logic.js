RightNow.namespace('Custom.Widgets.input.SelectionRadioInput');
Custom.Widgets.input.SelectionRadioInput = RightNow.Field.extend({
    /**
   ...
   /**
    ...
    _cloneSelectAsRadio: function() {
        var selectDiv = this.Y.one("#" + this.baseDomID + ' fieldset'); // We need to append the form elements to the fieldset inside the selectDiv
        if(selectDiv) {
			var html = '<div data-field="' + this._customDummyFieldName + '_field" id="' +
				this._customRadioDivName + '" role="radiogroup" aria-labelledby="' + 
				this.baseDomID + '_Label">';
			var myObject = document.getElementById(this._customFieldID);
 			...
 			for(var i = 0; i < myObject.options.length; i++) {
				if(myObject.options[i].value != "")	{
					...
					html += '<input type="radio" id="' + this._customRadioName + '-' + i +
						'"   name="' + this._customDummyFieldName + '"  value="' +
						myObject.options[i].value + '" ' + $selected + ' >';

					html += '<label for="' + this._customRadioName + '-' + i +'">' +
						myObject.options[i].text + '</label><br />';
				}
            }
