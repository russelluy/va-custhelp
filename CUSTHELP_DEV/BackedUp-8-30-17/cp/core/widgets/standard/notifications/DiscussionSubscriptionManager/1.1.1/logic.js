...
RightNow.Widgets.DiscussionSubscriptionManager = RightNow.Widgets.Multiline.extend({
    ...
    _addSubscription: function() {
        ...
        var focusElement = this.Y.one(this.baseSelector + '_AddButton') || this.Y.one(this.baseSelector + '_Content .rn_Discussions a');
    ...
    {
        if(this._errorDisplay) {
            var errorMessage = '<a href="javascript:void(0);" onclick="focusElement.focus(); return false;">' + message + '</a>';
