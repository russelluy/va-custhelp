...
RightNow.Widgets.VisualProductCategorySelector = RightNow.Widgets.extend({
    ...
    constructor: function() {
        ...
        if (this.widgetElement) {
            ...
            this._attachEventHandlers(this.widgetElement, [
                ['click', 'a.rn_ShowChildren', this.onShowChildrenClick],
                ['click', 'a.rn_BreadCrumbLink', this.goToPreviousLevel],
                ['click', 'a.rn_ForwardPage', this.goForwardPage],
                ['click', 'a.rn_PreviousPage', this.goPreviousPage]
            ]);
        }
    },
     ...
     */
    onShowChildrenClick: function(e) {
        e.halt();
        ...
        if (!isNaN(id)) {
            this.showChildren({ id: id, label: label, el: el });
        }
    },
    ...
    showChildren: function(parent) {
        ...
        if (subItemsAlreadyRendered) {
            ...
            this._setCurrentItemLevel(parent);
            this.insertNewGroupAtCurrentLevel(this.prefetched[parent.id]);
        }
        else if (this.requestChildren(parent.id, this.childrenResponse)) {
     ...
     */
    childrenResponse: function(response, origEventObject) {
        if (!response || !RightNow.Event.fire('evt_ItemResponse', { data: origEventObject, response: response })) return;
        this.insertNewGroupAtCurrentLevel(response.result[0]);
    },
     ...
     */
    insertNewGroupAtCurrentLevel: function (items) {
        var previousLevel = this.itemLevels[this.currentLevel - 1] || {};

        this.showNewGroup(items, this.Y.mix(previousLevel, { level: this.currentLevel + 1, focusOnFirst: true }));
    },
     ...
     */
    showNewGroup: function(items, options) {
        ...
        if(this.data.attrs.per_page > 0) {
            this._paginateItems(newItemGroup);
        }
        if (this.data.attrs.prefetch_sub_items) {
    ...
    /**
     * Set disabled status class for pagination
     *
     ...
     *
     * @param {Object} container Node object representing a collection of items (items)
     * @param {String=} direction Direction of pagination - either "forward" or "previous"
     */
    _paginateItems: function(container, direction) {
     ...
     * @param {Object} e Click event
     */
    goToPreviousLevel: function(e) {
        var id = parseInt(e.target.getAttribute('data-id'), 10);

        if (!this._showSubItemsFor(id)) {
            this.showChildren({ id: isNaN(id) ? this.itemLevels[0].id : id });
        }
     ...
     */
    paginateItemSet: function(e, direction) {
        e.halt();
        if(!e.currentTarget.hasClass('rn_Disabled')) {
            ...
            this._paginateItems(itemGroupContainer, direction);
        }
    },
    ...
    goPreviousPage: function(e) {
        this.paginateItemSet(e, 'previous');
    },
     ...
     * @param {Object} e Click event
     */
    goForwardPage: function(e) {
        this.paginateItemSet(e, 'forward');
    },
    ...
    _displayInitialItemSet: function(items) {
        ...
        this.showNewGroup(items, { id: id, level: ++level, focusOnFirst: false });
    },
    ...
    _updateBreadCrumb: function(level, itemLevels) {
        var toUpdate = [this.widgetElement.one('.rn_NavigationArea')],
            topLevelTitle = this.data.attrs.label_title,
            levels;
