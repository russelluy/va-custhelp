...
RightNow.Widgets.CombinedSearchResults = RightNow.Widgets.Multiline.extend({
     ...
     * @return {object|null} EventObject or null if no search sources are configured
    ...
    _searchAdditionalSources: function(evt, args) {
        ...
        var additionalSources = args[1] && args[1].w_id === this.data.info.w_id,
            page;
        args = args[0];
        ...
        if (additionalSources && (this.data.attrs.social_results)) {
            this._searching = true;
            this._searchedAdditionalSources = true;
        }
        else {
