    ...
    overrides: {
        ...
        validate: function(errors) {
            ...
            if (attrs.require_validation) {
                var validationLabel = RightNow.Text.sprintf(attrs.label_validation, attrs.label_error || attrs.label_input);

                this.toggleErrorIndicator(false, this.validation, this.validationLabel);
                if ((attrs.required || (this.data.js.requirements && 'length' in this.data.js.requirements)) && !this.validation.get('value')) {
                    errors.push({text: RightNow.Text.sprintf(attrs.label_required, validationLabel), id: this.validation.get('id')});
                    this.toggleErrorIndicator(true, this.validation, this.validationLabel);
                }
                else if (!this.validateValidation(true)) {
                    errors.push({text: RightNow.Text.sprintf(attrs.label_validation_incorrect, attrs.label_input, validationLabel)});
                    this.toggleErrorIndicator(true, this.validation, this.validationLabel);
                }
            }
