...
RightNow.Widgets.ModerationDateFilter = RightNow.SearchFilter.extend({
    ...
    _validate: function(evt, args) {
        ...
        if (this._isCustomDateSelected()) {
            ...
            if (!fromDate) {
                errors.push(this._getErrorMessage(fromDateInputID, this.data.attrs.label_invalid_from_date_error, this.data.js.date_format.label));
            }
            if (!toDate) {
                errors.push(this._getErrorMessage(toDateInputID, this.data.attrs.label_invalid_to_date_error, this.data.js.date_format.label));
            }
            if (fromDate && toDate) {
                var today = new Date();
