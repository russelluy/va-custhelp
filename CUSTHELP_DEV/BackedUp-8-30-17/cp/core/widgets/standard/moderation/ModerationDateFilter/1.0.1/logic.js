...
RightNow.Widgets.ModerationDateFilter = RightNow.SearchFilter.extend({
    ...
    _createCalendar: function(calendarConfig) {
        ...
        this.Y.one(calendarConfig.calIcon).on('click', function(ev) {
            ...
            var dateValue = dtdate.parse(dateInput) || now;
    ...
    _getDateFormat: function() {
        return ("%" + this.data.js.date_format).replace(/-/g, "-%")
                .replace(/\//g, "/%");
    },
     ...
     *
     */
    _isValidDate: function (dateVal) {
        var dateArray = dateVal.split(/[\/-]/);
        var dayobj = new Date(dateArray[2], dateArray[0] - 1, dateArray[1]);
        return ((dayobj.getMonth() + 1 == dateArray[0]) && (dayobj.getDate() == dateArray[1]) && (dayobj.getFullYear() == dateArray[2]));
    },
    ...
    _validate: function(evt, args) {
        ...
        if (this._isCustomDateSelected()) {
            var dtdate = this.Y.DataType.Date;
            var dates = this._getCustomDateInputs();
            var fromDate = dtdate.parse(dates.from);
            var toDate = dtdate.parse(dates.to);
            ...
            if (!fromDate || !this._isValidDate(dates.from)) {
                errors.push(this._getErrorMessage(fromDateInputID, this.data.attrs.label_invalid_from_date_error));
            }
            if (!toDate || !this._isValidDate(dates.to)) {
                errors.push(this._getErrorMessage(toDateInputID, this.data.attrs.label_invalid_to_date_error));
            }
            if (fromDate && toDate) {
                var today = new Date();
                ...
                if (dtdate.isGreater(minDate, fromDate)) {
                    errors.push(this._getErrorMessage(fromDateInputID, this.data.attrs.label_min_from_date_error));
                }
                if (dtdate.isGreater(minDate, toDate)) {
                    errors.push(this._getErrorMessage(toDateInputID, this.data.attrs.label_min_to_date_error));
                }
                ...
                $intervalParts = this.data.attrs.max_date_range_interval.split(" ");
                if($intervalParts[0] && $intervalParts[1]){
                    $unit = $intervalParts[1].charAt(0).toUpperCase() + $intervalParts[1].slice(1);
                    ...
                    if (!dtdate.isGreater(fromDate, toDate) && !dtdate.isInRange(toDate, fromDate, maxToDate)) {
                        errors.push(this._getErrorMessage(fromDateInputID, this.data.attrs.label_max_date_range_error, this.data.attrs.max_date_range_interval));
                    }
                }
            }
        }
