...
RightNow.Widgets.AnswerFeedback = RightNow.Widgets.extend({
    constructor: function() {
        ...
        this._rate = 0;
        if (!this.Y.one(this.baseSelector + "_FeedbackTextarea")) {
            RightNow.UI.DevelopmentHeader.addJavascriptError(RightNow.Text.sprintf(RightNow.Interface.getMessage("ANSWERFEEDBACK_DIALOG_MISSING_REQD_MSG"), this.baseDomID + "_FeedbackTextarea"));
            return;
        }
    ...
    _showDialog: function()
    {
        ...
        if (!this._dialog)
        {
            var buttons = [ { text: this.data.attrs.label_send_button, handler: {fn: this._onSubmit, scope: this}, isDefault: true},
                 ...
                 dialogForm = this.Y.one(this.baseSelector + "_AnswerFeedbackForm");
    ...
    */
        ...
        if(RightNow.Event.fire("evt_answerFeedbackResponse", response, originalEventObj)) {
            if(this._incidentCreateFlag){
                ...
                if(response && response.ID){
                    RightNow.UI.Dialog.messageDialog(this.data.attrs.label_feedback_submit, {exitCallback: {fn: this._closeDialog, scope: this}});
                }
                else{
                    var message = (response && response.error) ? response.error : response;
                    RightNow.UI.Dialog.messageDialog(message, {icon: "WARN", exitCallback: {fn: function(){ RightNow.UI.Dialog.enableDialogControls(this._dialog, this._keyListener); }, scope: this}});
                }
            }
            else{
                this._closeDialog();
    ...
    _replaceRatingElementsWithMessage: function(){
        ...
        if(ratingElement){
            var successMessage = this.Y.Node.create('<span class="rn_ThanksLabel">');
            successMessage.set('innerHTML', this.data.attrs.label_feedback_submit).set('tabIndex', -1);
            ratingElement.replace(successMessage);
