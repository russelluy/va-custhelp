        ...
        foreach ($this->data['attrs']['type'] as $activityType) {
            ...
            foreach ($activity as $action) {
                if (($activityType === 'comment' || $activityType === 'bestAnswerGivenByUser' || $activityType === 'bestAnswerGivenToUser') 
                    && !is_null($action->Parent->ID)
                    && (!($parent = $this->CI->model('SocialComment')->getTabular($action->Parent->ID)->result)
                    || in_array($parent->StatusWithType->StatusType->ID, $this->helper('Social')->getExcludedStatuses('SocialQuestionComment')))){
                    continue;
                }
