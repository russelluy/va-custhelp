    ...
    private function getCommentAndBestAnswerCounts() {
        ...
        if (!$totals) {
            ...
            while ($comment = $comments->next()) {
                if (!in_array($comment->SocialQuestion->StatusWithType->StatusType->ID,  $this->helper('Social')->getExcludedStatuses('SocialQuestion'))
                    && !in_array($comment->Parent->StatusWithType->StatusType->ID,  $this->helper('Social')->getExcludedStatuses('SocialQuestionComment'))) {
                    $commentCount++;
    ...
    private function getBestAnswerCount($comment) {
        ...
        foreach($comment->SocialQuestion->BestSocialQuestionAnswers ?: array() as $bestAnswer) {
            if ($comment->ID === $bestAnswer->SocialQuestionComment->ID
                ...
                && !in_array($bestAnswer->SocialQuestionComment->StatusWithType->StatusType->ID, $this->helper('Social')->getExcludedStatuses('SocialQuestionComment'))) {
                    $count++;
            }
        }
