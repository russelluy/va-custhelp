...
<div id="rn_<?=$this->instanceID;?>_Content" class="yui3-skin-sam">
    ...
    <caption><?=$data['attrs']['label_caption']?></caption>
            ...
            <tr>
             <? for ($i = 0; $i < count($data['fields']); $i++):?>
                <rn:block id="headerData">
                    ...
                    <? if($data['fields'][$i]['name'] === 'documentId' || $data['fields'][$i]['name'] === 'expires'): ?>
                        <? $headerClass .= ' yui3-datatable-sortable-column'; ?>
                    <? endif;?>
                    <th id="rn_<?=$this->instanceID;?>_<?= $data['fields'][$i]['name'] ?>" class="<?= $headerClass ?>" tabindex="0">
                        <?= $data['fields'][$i]['label'] ?>
                        <? if($data['fields'][$i]['name'] === 'documentId' || $data['fields'][$i]['name'] === 'expires'): ?>
        ...
        <tbody class="yui3-datatable-message">
            <tr>
                <td role="gridcell" colspan="<?=count($data['fields']);?>" class="yui3-datatable-message-content"><?=$data['attrs']['label_no_subscription']?></td>
            </tr>
        </tbody>
        <? endif;?>
    </table>
