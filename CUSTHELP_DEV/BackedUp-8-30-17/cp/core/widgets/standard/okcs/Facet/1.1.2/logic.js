...
RightNow.Widgets.Facet = RightNow.SearchConsumer.extend({
    ...
    */
    _processChildren : function(currentFacet, hasChildren) {
        ...
        item.append(this.Y.Node.create("<a role='button' class='" + facetLinkClass + "' id='" + currentFacet.id + "' href='javascript:void(0)'>" + spanExp + currentFacetDescription + "</a>"));
        ...
        for (var i = 0; i < len; ++i) {
            ...
            if (currFacet !== undefined) {
                if (currFacet.children.length !== 0) {
                    ...
                    if (maxFacetLength === undefined && maxFacetLength !== '')
