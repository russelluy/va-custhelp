...
class AnswerList extends \RightNow\Widgets\SourceResultListing {
    ...
    function getData() {
        if (!$this->helper('Okcs')->checkOkcsEnabledFlag($this->getPath())) {
            $this->data['errorDetail'] = array('errorCode' => '','externalMessage' => \RightNow\Utils\Config::getMessage(THE_OKCSENABLED_CFG_SET_MUST_BE_MSG));
            return false;
        }
            ...
            if ($articles->error === null) {
                ...
                if ($isTable) {
                    ...
                    if (!empty($this->data['fields'])) {
                        foreach ($articles->items as $document) {
                            foreach ($this->data['fields'] as $field) {
                                if (!property_exists($document, $field['name']) && $document->$field['name'] === null) {
                                    ...
                                    $this->data['errorDetail'] = array('errorCode' => '','externalMessage' => \RightNow\Utils\Config::getMessage(RES_OBJECT_PROPERTY_PCT_S_IS_NOT_MSG));
                                    return false;
                                }
                            }
                            $item = array(
                        ...
                        $this->data['errorDetail'] = array('errorCode' => '','externalMessage' => \RightNow\Utils\Config::getMessage(MISSING_TITLE_IN_DISPFLDS_ADD_LBL));
                        return false;
                    }
                }
                ...
                $this->data['errorDetail'] = array('errorCode' => $articles->error->errorCode,'externalMessage' => $articles->error->externalMessage);
                return false;
            }
        }
    }
