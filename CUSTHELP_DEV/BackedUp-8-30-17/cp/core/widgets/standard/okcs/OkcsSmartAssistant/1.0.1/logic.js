...
RightNow.Widgets.OkcsSmartAssistant = RightNow.Widgets.SmartAssistantDialog.extend({
    overrides: {
         ...
         */
        _getAnswerContent: function(clickedLink, eventObject) {
            var highlightUrl = clickedLink.getAttribute('data-highlightUrl'),
                ...
                clickthroughEventObject = new RightNow.Event.EventObject(this, {data: {
                    docID: answerDetails[0],
                    answerID: answerID,
                    type: true,
                    clickThroughLink: clickedLink.getAttribute('data-clickThroughUrl'),
                    iqAction: answerDetails[2]
                }});

            RightNow.Ajax.makeRequest(this.data.attrs.get_okcs_data_ajax, clickthroughEventObject.data,
                { ignoreFailure: true, data: clickthroughEventObject, scope: this }
            );
            ...
            if(highlightUrl !== undefined && highlightUrl !== '' ) {
                ...
                eventObject = new RightNow.Event.EventObject(this, {data: {doc_id: answerType === 'HTML' ? answerID : imDocID}});
            }
            this._displayOKCSAnswerContent(answerID, clickedLink.getAttribute('data-url'), clickedLink.getAttribute('title'), eventObject);
        },

        /**
        * Common method to insert the content of an answer
        * @param {Object} answerWrapper container of the answer content
        */
        _insertAnswer: function(answerWrapper) {
            if (this._isExplorerView) {
                this._dialogContent.insert(answerWrapper, "after");
            }
            else {
                this._answer = this.Y.one("#" + this.baseDomID + "_Answer" + this._showingAnswer.getAttribute('data-id'));
                this._answer.insert(answerWrapper, "after");
            }
        },
        ...
        */
        _expandAnswerContent: function(id, answerID, answer, toggle, alt) {
            for(var i in this._answersLoaded) {
                if(this._answersLoaded.hasOwnProperty(i) && i !== answerID && this._answersLoaded[i]) {
    ...
    * @param answerID string ID of an OKCS IM document
    * @param answerUrl string URL of an OKCS IM document
    * @param answerTitle string title of an OKCS IM document.
    * @param eventObject object Event
    */
    _displayOKCSAnswerContent: function(answerID, answerUrl, answerTitle, eventObject) {
        if(RightNow.Event.fire("evt_getAnswerResponse")){
            if(this._showingAnswer && this._showingAnswer.getAttribute("data-id").indexOf(answerID) > -1 && answerID) {
    ...
    _displayExternalResults: function(response, data) {
        var content = this.Y.Node.create("<div class='rn_ExternalContent' id='" + this.baseDomID + "_Div" + data.answerID + "'></div>");
        content.set('innerHTML', response.contents);
    ...
    */
    _showOkcsContent: function(answerID, answerContent) {
        var answerWrapper = this.Y.Node.create("<span id='" + this.baseDomID + "_AnswerContent" + answerID + "' class='rn_Answer rn_AnswerDetail rn_Hidden'></span>"),
    ...
    _explorerViewToggleAnswerContent: function(answerID, expand, link, answerContent) {
        ...
        if (expand) {
            ...
            this._hideButton.once("click", function(){
                this._toggleAnswerContent(answerID, false);
            }, this);
            this._newTabButton = this.Y.one(this.baseSelector + "_NewTabButton" + answerID);
            this._newTabButton.once("click", this._openNewTab, this);
