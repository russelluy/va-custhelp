...
RightNow.Widgets.SearchResult = RightNow.SearchConsumer.extend({
    ...
    */
    ...
    _onReportChanged: function(type, args) {
        ...
        if (args[0] !== undefined && args[0].data.error !== undefined) {
            ...
            if (results && results.length > 0) {
                ...
                newContent = new EJS({text: this.getStatic().templates.view}).render({data: results[0].resultItems, title: this.data.attrs.label_results, answerPageUrl: this._answerPageUrl, fileDescription: this.data.js.fileDescription, searchSession: this._okcsSearchSession, transactionID: this._transactionID, getUrlData: this.getUrlData, yuiObj: this.Y, widgetInstanceID: this.baseDomID});
            }
            else {
    ...
    onResultClick: function(evt) {
        ...
        var navigationUrl = dataHref.indexOf("#__highlight") > 0 ? (dataHref.substring(0, dataHref.indexOf("#__highlight")) + txnParam + "#__highlight") : (dataHref + txnParam);
        if(this.data.attrs.target === '_blank')
