...
RightNow.Widgets.ConditionalChatLink = RightNow.Widgets.extend({
    ...
    _startPollingTimer: function(startImmediately, args)
    {
        ...
        if(chatData.length !== 0)
            this._linkUrl = RightNow.Url.addParameter(this.data.js.link_url, 'chat_data', chatData);
    ...
    {
        if(result && result.stats && RightNow.Event.fire("evt_chatQueueResponse", {data:{w_id: this._eo.w_id, name:"ConditionalChatLink"}, response: {stats:{expectedWaitSeconds:parseInt(result.stats.expectedWaitSeconds, 10)}}}))
        {
