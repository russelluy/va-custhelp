...
RightNow.Widgets.BestAnswerDisplay = RightNow.Widgets.extend({
    ...
    _bestAnswerUpdated: function(response, origEventObj) {
        if(!origEventObj[0].errors) {
            this._displayRefreshBanner(this.el);
    ...
    /**
     * Pops the RefreshBanner view into the given element
     * if the given element isn't currently visible within
     * the viewport.
     * @param  {Object} parent Node to inject the view into
     */
    _displayRefreshBanner: function(parent) {
        if (!this._elementIsInViewport(parent)) {
            RightNow.UI.displayBanner(this.data.attrs.label_dynamic_notice).on('click', this._scrollToWidget, this);
        }
    },

    /**
     * Determines whether the element is inside the
     * viewport region. Only cares about vertical position.
     * @param  {Object} el Node to check
     * @return {Boolean}    T if in viewport, F if outside
     */
    _elementIsInViewport: function (el) {
        var y = el.get('offsetTop') + el.get('offsetHeight'),
            viewport = this.Y.DOM.viewportRegion();

        return y >= viewport.top && y <= viewport.bottom;
    },
