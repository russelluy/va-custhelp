        ...
        <div class="rn_BestAnswerBody rn_CommentCollapsed">
            ...
            <div class="rn_BestAnswerActions">
                ...
                <? if($userType = $this->helper->shouldDisplayBestAnswerRemoval($bestAnswer['comment']['data'])): ?>
                    <? if ($userType === "Author" || $userType === "Both"): ?>
                        <span class="rn_UserTypeAuthor">
                            <button type="button" data-commentid="<?= $bestAnswer['comment']['data']->ID ?>" title="<?= $this->data['attrs']['label_unmark_best_answer_author_title'] ?>">
                    ...
                    <? if ($userType === "Moderator" || $userType === "Both"): ?>
                        <span class="rn_UserTypeModerator">
                            <button type="button" data-commentid="<?= $bestAnswer['comment']['data']->ID ?>" title="<?= $this->data['attrs']['label_unmark_best_answer_moderator_title'] ?>">
                ...
                <div class="rn_BestAnswerCommentActions">
                    <a itemprop="url" class="rn_ReplyToComment <?= $this->data['attrs']['always_show_jump_to'] ? '' : 'rn_Hidden' ?>" role="button" href="<?= $this->data['attrs']['jump_to_link_url'] ?>/qid/<?= $question->ID ?>/comment/<?= $bestAnswer['comment']['data']->ID ?>" data-commentid="<?= $bestAnswer['comment']['data']->ID ?>">
                        <?= $this->data['attrs']['label_jump_to'] ?>
                    </a>
                    <a class="rn_ShowAllCommentText" role="button" href="javascript:void(0)" data-commentid="<?= $bestAnswer['comment']['data']->ID ?>">
                        <?= $this->data['attrs']['label_expand'] ?>
                    </a>
                    <a class="rn_CollapseCommentText rn_Hidden" role="button" href="javascript:void(0)" data-commentid="<?= $bestAnswer['comment']['data']->ID ?>">
