...
RightNow.Widgets.QuestionComments = RightNow.Widgets.extend({
    ...
    _newCommentResponse: function(response, origResponseObj) {
        ...
        if (this.data.attrs.label_new_comment_banner) {
            ...
            if (newComments) {
                var lastCommentID = newComments.one(' > .rn_CommentContainer:last-child').getAttribute('data-commentid');
                RightNow.UI.displayBanner(this.data.attrs.label_new_comment_banner, { focus: true })
                    .on('click', this.Y.bind(this.readingPosition.fire, this.readingPosition, 'refresh', { commentID: lastCommentID }));
            }
        }
    ...
    _editCommentResponse: function(response, originalEventObj) {
        ...
        if(this.data.attrs.label_edit_comment_banner) {
            RightNow.UI.displayBanner(this.data.attrs.label_edit_comment_banner, { focus: true })
                .on('click', this.Y.bind(this.readingPosition.fire, this.readingPosition, 'refresh', { commentID: originalEventObj.data.commentID }));
        }
    ...
    _replyToCommentResponse: function(response, originalEventObj) {
        ...
        if (label = this.data.attrs.label_replied) {
            RightNow.UI.displayBanner(label);
        }
    ...
    _deleteCommentResponse: function(response, originalEventObj) {
        ...
        RightNow.UI.displayBanner(this.data.attrs.label_delete_comment_banner);
        this._toggleNewCommentAction();
        this._checkBestAnswerComment(originalEventObj.data.commentID);
    },
            ...
            if (RightNow.Event.fire(eventName + 'Response', response, originalEventObj)) {
                ...
                if (error) {
                    if(eventName.indexOf('bestAnswer') > -1) {
                        this.Y.one(this.baseSelector + '_' + originalEventObj.data.commentID + ' .rn_BestAnswerActions .rn_UserType' + originalEventObj.data.chosenByType + ' button').set('disabled', false);
                    }
                    ...
                    if(!RightNow.Ajax.indicatesSocialUserError(response)) {
                        RightNow.UI.displayBanner(RightNow.Interface.getMessage("UNABLE_SAVE_YOUR_COMMENT_AT_THIS_TIME_LBL"), { type: 'ERROR', focus: true });
                    }
    ...
    _checkBestAnswerComment: function(commentID) {
        if(this.Y.one('.rn_BestAnswerList .rn_ShowAllCommentText[data-commentid=' + commentID + ']')) {
            RightNow.Event.fire('evt_refreshBestAnswer', new RightNow.Event.EventObject(this));
        }
    }
});
    ...
    */
    ...
    editComment: function(commentContainer, commentText) {
        return this._orchestrateMove('edit', {
            ...
            shouldMove: function() {
                return !this.isSiblingOfNode(commentContainer);
            },
...
RightNow.Widgets.QuestionComments.commentActions = RightNow.EventProvider.extend({
    ...
    _shareClicked: function(e) {
        this._showPanel(e.currentTarget, this._getUrl(this._getCommentID(e.currentTarget)));
    },
