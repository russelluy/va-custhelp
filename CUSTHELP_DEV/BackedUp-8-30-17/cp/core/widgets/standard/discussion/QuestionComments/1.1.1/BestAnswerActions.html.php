<? $userTypeMarking = $this->helper->shouldDisplayBestAnswerActions($comment, $this->data['socialUser'], true);
   $userTypeRemoving = $this->helper->shouldDisplayBestAnswerActions($comment, $this->data['socialUser'], false); ?>
    ...
    <? if ($userTypeMarking): ?>
        <? if ($userTypeMarking === "Author" || $userTypeMarking === "Both"): ?>
            <span class="rn_BestAnswerAssignment rn_UserTypeAuthor">
                <button type="button" data-commentid="<?= $comment->ID ?>" title="<?= $this->data['attrs']['label_mark_best_answer_author_title'] ?>">
        ...
        <? if ($userTypeMarking === "Moderator" || $userTypeMarking === "Both"): ?>
            <span class="rn_BestAnswerAssignment rn_UserTypeModerator">
                <button type="button" data-commentid="<?= $comment->ID ?>" title="<?= $this->data['attrs']['label_mark_best_answer_moderator_title'] ?>">
    ...
    <? if ($userTypeRemoving): ?>
        <? if ($userTypeRemoving === "Author" || $userTypeRemoving === "Both"): ?>
            <span class="rn_BestAnswerRemoval rn_UserTypeAuthor">
                <button type="button" data-commentid="<?= $comment->ID ?>" title="<?= $this->data['attrs']['label_unmark_best_answer_author_title'] ?>">
        ...
        <? if ($userTypeRemoving === "Moderator" || $userTypeRemoving === "Both"): ?>
            <span class="rn_BestAnswerRemoval rn_UserTypeModerator">
                <button type="button" data-commentid="<?= $comment->ID ?>" title="<?= $this->data['attrs']['label_unmark_best_answer_moderator_title'] ?>">
