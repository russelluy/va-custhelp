...
class QuestionCommentsHelper extends \RightNow\Libraries\Widget\Helper {
    ...
    function formattedTimestamp ($timestamp) {
        return \RightNow\Libraries\Formatter::formatDateTime(is_int($timestamp) ? $timestamp : strtotime($timestamp));
    }
    ...
    function shouldDisplayCommentReply($forComment) {
        if (!Framework::isLoggedIn()) {
            return $this->shouldDisplayNewCommentArea();
        }
     ...
     */
    function shouldDisplayBestAnswerActions($forComment, $socialUser, $isMarking) {
