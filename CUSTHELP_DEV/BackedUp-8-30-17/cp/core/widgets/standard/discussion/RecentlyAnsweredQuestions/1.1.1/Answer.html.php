...
<div class="rn_AnswerInfo <?= $className ?>" itemprop="suggestedAnswer acceptedAnswer" itemscope itemtype="http://schema.org/Answer">
    ...
    <div class="rn_AnswerContent">
        <div class="rn_AnswerText" itemprop="text">
            ...
            <?= \RightNow\Utils\Text::truncateText(\RightNow\Libraries\Formatter::formatMarkdownEntry($comment->Body), $this->data['attrs']['answer_text_length']) ?>
        </div>
        <div class="rn_AnswerMoreLink">
            <a href="<?= $link ?>#comment_<?= $comment->ID ?>" itemprop="discussionUrl"><?= $this->data['attrs']['label_answer_more_link'] ?></a>
        </div>
    </div>
</div>
