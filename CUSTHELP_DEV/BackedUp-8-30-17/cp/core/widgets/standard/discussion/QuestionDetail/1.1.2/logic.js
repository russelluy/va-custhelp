...
RightNow.Widgets.QuestionDetail = RightNow.Widgets.extend({
    ...
    _deleteQuestionResponse: function(response, args) {
        if(!response.errors && RightNow.Event.fire('evt_deleteQuestionResponse', response, args[0])) {
            RightNow.UI.displayBanner(this.data.attrs.successfully_deleted_question_banner, { type: 'SUCCESS', focus: true }).on('close',function() {
            ...
            RightNow.UI.displayBanner(RightNow.Interface.getMessage("THERE_WAS_AN_ERROR_DELETING_QUESTION_LBL"), { type: 'ERROR', focus: true });
        }
    }
});
