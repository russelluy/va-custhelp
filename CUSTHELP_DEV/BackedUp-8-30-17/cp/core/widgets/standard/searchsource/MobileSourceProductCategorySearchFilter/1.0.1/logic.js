...
RightNow.Widgets.MobileSourceProductCategorySearchFilter = RightNow.SearchProducer.extend({
    overrides: {
        constructor: function() {
            ...
            this.Y.Event.attach("click", this.openDialog, this._baseID + "_Launch", this);
            this.Y.Event.attach("click", function() {
                //filter removal handler
               this.clearSelection();
               if(this.data.js.linkingOn && this.data.attrs.filter_type === "Product") {
                   RightNow.Event.fire("evt_menuFilterRequest", this.updateEventObject({data: {level: this._currentLevel, value: -1}}));
               }
            ...
            RightNow.Event.subscribe("evt_menuFilterGetResponse", this.getSubLevelResponse, this);
            this.searchSource(this.data.attrs.report_id)
                .on('collect', this.collectFilters, this)
                .on("response", this.onSearchResponse, this)
    ...
    */
    createDialog: function(element) {
        element || (element = this.Y.one(this._baseID + "_Level1Input"));
        if (!element) return;
        this._dialog = RightNow.UI.Dialog.actionDialog(this.data.attrs.label_prompt, element, {navButtons: true, cssClass: "rn_MobileSourceProductCategorySearchFilter"});
        ...
        element.removeClass("rn_Hidden").delegate("click", this.getSubLevelRequest, "input", this);
    },
     ...
     */
    selectionAllMade: function(parentSelected) {
        this.selectionMade(parentSelected);
        if (this.data.js.linkingOn && this.data.attrs.filter_type === "Product") {
    ...
    * @param {Number} parentSelected ID of the parent being selected
    */
    selectionMade: function(parentSelected) {
        ...
        if(filterDiv) {
            ...
            //construct new nodes
            var anchor, urlParams = "", selected, page = this.data.js.searchPage + '/' + this.data.js.filter.key + '/';
            do {
        ...
        this.markSelectionInDialog();
        this.commitSelection(this._selections);
        //update the selection for the filter request event
        this.updateEventObject({data: {level: this._currentLevel, value: this._currentValue}, filters: {data: {0: eventData, reconstructData: eventReconstructData}}});

        // event to notify listeners of selection
        this._eo.data.hierChain = this.getCommittedSelection(true);
    ...
    */
    markSelectionInDialog: function(deselectAll) {
        if (deselectAll) {
     ...
     */
    getSubLevelRequest: function(clickEvent) {
        if (!this._beingSelected) {
            this.toggleLoadingIcon(clickEvent.target);
            ...
            if (this.data.js.linkingOn && this.data.attrs.filter_type === "Product") {
                ...
                RightNow.Event.fire("evt_menuFilterRequest", this.updateEventObject({data: {level: this._currentLevel, value: this._currentValue}}));
                if (!hasChildren) {
    ...
    */
    buildDialogContent: function(data, level) {
        if(data.length && level < 7 && this._dialog) {
            ...
            if (existingElement) {
                ...
                var committedSelections = this.getCommittedSelection();
                ...
                if(!isSameSelection && !(committedSelections.length + 1 === this._currentLevel && committedSelections[committedSelections.length - 1].value === this._currentValue)) {
                    ...
                    this.markSelectionInDialog(existingElement);
                }
                ...
                var currentSelections = this.getCommittedSelection(),
                ...
                this.Y.one(id).delegate("click", function(e) {
                    if (e.target.compareTo(allItem)) {
                        this.selectionAllMade(data[0].id);
                    }
                    else {
                        this.getSubLevelRequest(e);
                    }
                }, "input", this);
            }
        }
    ...
    */
    buildLinkedCategoryContent: function(level, data, selectedItem) {
        ...
        this.createDialog(firstLevelItems);
    ...
    getSubLevelResponse: function(type, args) {
        ...
        if((evtObj.w_id && evtObj.w_id === this.instanceID) || (this.data.js.linkingOn && evtObj.data.data_type === "Category" && this.data.attrs.filter_type === evtObj.data.data_type)) {
            if(evtObj.data.reset_linked_category) {
                ...
                if (!evtObj.data.hier_data.length) {
                    ...
                    if (this._selections.length) {
                        ...
                        if(!stillSelected) {
                            this.clearSelection();
                            this.buildLinkedCategoryContent(1, evtObj.data.hier_data, stillSelected);
                        }
                        else {
                            var firstLevelItems = this.buildLinkedCategoryContent(1, evtObj.data.hier_data, stillSelected),
                                baseID = this._baseID,
                                thisY = this.Y,
            ...
            if(evtObj.data.hier_data.length === 0) {
                ...
                this.buildDialogContent(evtObj.data.hier_data, this._currentLevel);
            }
            this.toggleLoadingIcon();
        }
    },
    ...
    */
    commitSelection: function(selection) {
        this._committedSelection = RightNow.Lang.cloneObject(selection);
    },

    /**
    * Returns the selection hierarchy stored via commitSelection().
    ...
    */
    getCommittedSelection: function(justValues) {
    ...
    * @param {Array} args Event arguments
    */
    onSearchResponse: function(type, args) {
        //if you don't care about prod/cat linking skip this condition...
        if(this.data.js.linkingOn && this.data.attrs.filter_type === "Product" && typeof this._currentValue !== "undefined") {
            RightNow.Event.fire("evt_menuFilterRequest", this.updateEventObject({data: {level: this._currentLevel, value: this._currentValue || -1}}));
        }
    },
    ...
    * @param options Object intended to represent event object structure
    * ({data: {...}, filters: {...}})
    */
    updateEventObject: function(options) {
        var widgetData = this.data.js;
        if(!this._initialized) {
            this._eo = new RightNow.Event.EventObject(this, {
                data: {
                    key: widgetData.filter.key,
                    type: widgetData.filter.type,
                    cache: [],
            ...
            this.updateEventObject._updateVals =
        ...
        if(options) {
            if(options.data) {
                this.updateEventObject._updateVals(this._eo.data, options.data);
            }
            if(options.filters && options.filters.data) {
                this.updateEventObject._updateVals(this._eo.filters.data, options.filters.data);
            }
        }
    ...
    */
    toggleLoadingIcon: function(toggleNode) {
        if(toggleNode && !toggleNode.hasClass('rn_LoadingIcon')) {
            ...
            var existingLoadingIcon = this.Y.one('.rn_MobileSourceProductCategorySearchFilter .rn_LoadingIcon');
