...
class PhpFunctional extends \RightNow\Controllers\Admin\Base {
    ...
    function __construct() {
        ...
        $this->sessionString = '/session/' . base64_encode("/time/$time/sid/" . $this->sessionID);
        $session->setSessionData(array('sessionString' => $this->sessionString));
        $this->sessionCookie = Api::ver_ske_encrypt_fast_urlsafe(json_encode(array('s' => $this->sessionID, 'e' => $this->sessionString, 'l' => $time, 'i' => Api::intf_id())));
    }
