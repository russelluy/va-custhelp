    ...
    private function getContactCookies($testData) {
        ...
        if ($contactLogin) {
            if (!$this->contactCookies[$contactLogin]) {
                ...
                $sessionID = $profile->sessionid; // Get the new, real value here.
                $session = \RightNow\Libraries\Session::getInstance();
                $profile = $session->createMapping($profile);
                $profile = json_encode(array('c' => $profile->cookie, 'p' => $profile->pta_login_used, 'i' => Api::intf_id()));
        ...
        if ($sessionData) {
            ...
            if ($cached = $this->sessionCookies[$sessionKey]) {
                ...
                $session = new \RightNow\Libraries\SessionData(array(
                    's' => $sessionID,
                    'a' => (int) $sessionData['answersViewed'] ?: 0,
                    'n' => (int) $sessionData['numberOfSearches'] ?: 0,
                    'u' => $sessionData['urlParameters'] ?: array(),
                    'p' => (bool) $sessionData['ptaUsed'] ?: false,
                    'e' => '/session/' . base64_encode("/time/$time/sid/" . $sessionID),
                    'r' => $sessionData['previouslySeenEmail'] ? Text::escapeHtml($sessionData['previouslySeenEmail']) : null,
                    'l' => $time,
                    'i' => Api::intf_id(),
                ));
