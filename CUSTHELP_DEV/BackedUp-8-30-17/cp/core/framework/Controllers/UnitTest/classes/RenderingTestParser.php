...
class RenderingTestParser {
    ...
    private static $testKeywords = array(
        ...
        'postSql',
        // Any session keys and values to be used to request the page. Example: `session: [answersViewed: 2, numberOfSearches: 1, urlParameters: p, ptaUsed: true, previouslySeenEmail: bananas@banana.ape]`
        'session',
    );
        ...
        foreach (file($inputFile) as $line) {
            if (preg_match($this->getTestKeywordRegex(), $line, $matches)) {
                ...
                if (Text::beginsWith($arrayLike, '[') && Text::endsWith($arrayLike, ']') &&
                        preg_match_all('/\s*(\w*)\s*:\s*([a-z@.A-Z0-9\'\"]*)\s*,?/', $arrayLike, $matches)) {
                    // Check if it's an array-looking thing and convert it into a real array if so.
                    // e.g. [hey: oh, blah: blech]
                    $arrayVal = array();
                    for ($i = 0; $i < count($matches[1]); $i++) {
                        $arrayVal[$matches[1][$i]] = $matches[2][$i];
                    }
                    $sections[$currentKeyword] = $arrayVal;
                }
            }
