...
class Helper {
     ...
     * @return function
     */
    static function getMethodInvoker($className, $methodName, array $constructorArgs = array()) {
    ...
    static function makeRequest($url, $options = array(), $verbose = false) {
        ...
        $command = "wget $headers"
            ...
            . "'http" . ($options['useHttps'] ? 's' : '') . '://' . Config::getConfig(OE_WEB_SERVER) . "{$url}' 2>&1";
     ...
     * @return String response
     */
    public static function sendFile($url, $fileLocation, $contentType = 'application/octet-stream', $includeHeaders = false) {
        if (self::$verbose) {
            printf("Sending %s to %s%s <br><br>", $fileLocation, Url::getShortEufBaseUrl(), $url);
        }

        $data = array('file' => "@$fileLocation;type=$contentType");

        self::loadCurl();
        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, Url::getShortEufBaseUrl() . $url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Host' => Url::getShortEufBaseUrl()));
        ...
        if ($result !== false && $includeHeaders) {
        ...
        curl_close($ch);
        return $result !== false && $includeHeaders ? array($header, $body) : $result;
    }
    ...
    static function logInUser($userName = 'slatest') {
        ...
        $rawSession = $session = new \RightNow\Libraries\SessionData($sessionData);

        return array(
            'profile' => urlencode(Api::ver_ske_encrypt_urlsafe(json_encode($profile))),
            'session' => urlencode(Api::ver_ske_encrypt_fast_urlsafe(json_encode($session->convertToCookie()))),
