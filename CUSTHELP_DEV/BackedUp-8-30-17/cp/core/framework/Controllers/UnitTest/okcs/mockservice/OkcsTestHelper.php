...
class OkcsTestHelper
{
    ...
    */
    ...
    public static function getPayloadInfo(){
        ...
        return $input;
        //do a json_decode( if you are expecting a json payload
    }
    ...
    */
    ...
    public static function getFileContents($fileName){
        ...
        $x = self::isUserAuthenticated()?'authenticated':'guest';
        
        if(file_exists(CPCORE.'Controllers/UnitTest/okcs/mockservice/km/api/'.$queryParameterArray[1].'/')){
            $FilePath = CPCORE.'Controllers/UnitTest/okcs/mockservice/km/api/'.$queryParameterArray[1].'/'.$x.$fileName;
        }
        else{
            $FilePath = CPCORE.'Controllers/UnitTest/okcs/mockservice/km/api/latest/'.$x.$fileName;
        }
        if(file_exists ($FilePath))
        {
            $FileContents = fopen($FilePath, "r") or die("Unable to open file!");
    ...
    */
    public static function html_response_code($code){
        switch($code) {
            case 200: header("Content-Type:application/json", false);
                      break;
            case 400: header(' ',true,400);
                      header("Content-Type:application/json", false);
                      echo '{"statusCode":"400","error":{"message":"Bad Request"}}'; 
                      break;
            case 401: echo '<h1>Unauthorized</h1>';
                      break;
            case 402: echo '<h1>Payment Required</h1>';
                      break;
            case 403: echo '<h1>Forbidden</h1>';
                      break;
            case 404: header(' ',true,404);
                      header("Content-Type:application/json", false);
                      echo '{"statusCode":"404","error":{"message":"Expected object not found. Cause: Key not found","code":"OK-GEN0002","type":"VALIDATION"}}';
                      break;
            case 405: header(' ',true,405);
                      header("Content-Type:application/json", false);
                      echo '{"statusCode":"405","error":{"message":"Method not allowed","code":"OK-GEN0002","type":"APPLICATION"}}';
                      break;
            case 406: echo '<h1>Not Acceptable</h1>';
                      break;
            case 407: echo '<h1>Proxy Authentication Required</h1>';
                      break;
            case 408: echo '<h1>Request Time-out</h1>';
                      break;
            case 409: echo '<h1>Conflict</h1>';
                      break;
            case 410: echo '<h1>Gone</h1>';
                      break;
            case 411: echo '<h1>Length Required</h1>';
                      break;
            case 412: echo '<h1>Precondition Failed</h1>';
                      break;
            case 413: echo '<h1>Request Entity Too Large</h1>';
                      break;
            case 414: echo '<h1>Request-URI Too Large</h1>';
                      break;
            case 415: echo '<h1>Unsupported Media Type</h1>';
                      break;
            case 500: header(' ',true,500);
                      header("Content-Type:application/json", false);
                      echo '{"statusCode":"500","error":{"message":"Internal Server Error"}}'; 
            case 501: echo '<h1>Not Implemented</h1>';
                      break;
            case 502: echo '<h1>Bad Gateway</h1>';
                      break;
            case 503: echo '<h1>Service Unavailable</h1>';
                      break;
            case 504: echo '<h1>Gateway Time-out</h1>';
                      break;
            case 505: echo '<h1>HTTP Version not supported</h1>';
                      break;
             default:
    ...
    public static function updateMockAnswerResults($okcsImApiUrl,$rangeArray){
        ...
        foreach($rangeArray as $i){
                ...
                echo $FilePath;
                $handle = fopen($FilePath, "wb") or die('Cannot open file:  '.$my_file);
                fwrite($handle, $resp); 
                fclose($handle);
                curl_close($curl);
        }
