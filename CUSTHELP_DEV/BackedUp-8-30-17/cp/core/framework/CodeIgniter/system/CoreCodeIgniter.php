...
class CI_Router {
    ...
    function setVariablesFor404Page()
    {
        ...
        $sessionParameterSegment = array_search('session', $this->segments, true);
        $sessionValue = null;
        //If the session parameter was found, grab the value in the next segment
        if($sessionParameterSegment !== false)
            $sessionValue = $this->segments[$sessionParameterSegment + 1];
        if($sessionValue)
...
class CI_Input {
    ...
    /**
     * Fetch an item from either GET or POST
     *
     * @access    public
     * @param    string
     * @param    bool
    ...
    function request($index = '', $xssClean = true)
    {
        ...
        {
            if ($xssClean)
                return str_replace('"', '&quot;', $this->post($index));
            return $this->post($index);
        }
        $getValue = \RightNow\Utils\Url::getParameter($index);
        if ($getValue === null)
            return false;
        if ($xssClean)
           return $getValue;
        return str_replace("&quot;", '"', $getValue);
    }
...
class Themes {
    ...
    private static function getSpecificReferencePath($themeName){
        ...
        return IS_HOSTED ? '/euf/core/' . CP_FRAMEWORK_VERSION . '.' . CP_FRAMEWORK_NANO_VERSION . "/default/themes/$themeName" : constant("self::{$localThemeVariable}");
    }
}
...
class Rnow
{
    ...
    function __construct($fullInitialization = true)
    {
        ...
        // Use the new API call to validate the address if a define used with that code exists
        if (defined('RET_ACCESS_VALIDATE_SUCCESS')) {
            self::validateRemoteAddressNew();
        }
        else {
    ...
    private static function validateRemoteAddress() {
        ...
        if (MOD_ACCESS === MOD_ADMIN && !$forceModPublic) {
            $validHosts = self::getConfig(SEC_VALID_ADMIN_HOSTS);
        }
        elseif (MOD_ACCESS === MOD_PUBLIC || $forceModPublic) {
            $validHosts = self::getConfig(SEC_VALID_ENDUSER_HOSTS);
            $invalidHosts = self::getConfig(SEC_INVALID_ENDUSER_HOSTS);
            $invalidUAs = self::getConfig(SEC_INVALID_USER_AGENT);
        }

        // [ipaddr,ua]_validate() assume whitelist, invert returns if using do_not_suggest
        $validHostFail = $validHosts && (!Api::ipaddr_validate($validHosts, $_SERVER['REMOTE_ADDR']) ? !Api::ipaddr_validate($validHosts, $_SERVER['HTTP_X_FORWARDED_FOR']) : 0);
        $invalidHostFail = isset($invalidHosts) && $invalidHosts && (!$_SERVER['REMOTE_ADDR'] || Api::ipaddr_validate($invalidHosts, $_SERVER['REMOTE_ADDR']));
        $invalidUAFail = isset($invalidUAs) && $invalidUAs && (!$_SERVER['HTTP_USER_AGENT'] || Api::ua_validate($invalidUAs, $_SERVER['HTTP_USER_AGENT']));

        if ($validHostFail || $invalidHostFail || $invalidUAFail) {
            if (!$_SERVER['REMOTE_ADDR']) {
                $errorMessage = self::getMessage(NO_CLIENT_ADDR_SPEC_MSG);
            }
            elseif ($validHostFail || $invalidHostFail) {
...
HTML;
    ...
    /**
     * New version of validateRemoteAddress to be used for CX versions that have story 150305-000038 in them.
     */
    private static function validateRemoteAddressNew() {
        $forceModPublic = func_num_args() > 0 ? func_get_arg(0) : false;
...
HTML;
    ...
    private static function postCommonApiInit($fullInitialization) {
        ...
        // The following code is a hack set up to execute for CP 3.2 in 16.5 only. There is an issue in 16.5 where service pack and upgrade
        // deploys submitted by HMS fail due to linkage issues caused by the Turkish language have two versions of the
        // letter 'i'. (The code that breaks was replaced in 16.8.) If we detect a service pack of upgrade deploy happening 
        // on a Turkish interface we will switch to English so everything will run.

        if ((setlocale(LC_CTYPE, null) === 'tr_TR') && (\RightNow\Internal\Utils\Version::compareVersionNumbers(\RightNow\Internal\Utils\Version::getVersionNumber(MOD_BUILD_VER), "16.5") === 0)) {
            $uri = strtolower(urldecode(ORIGINAL_REQUEST_URI));
            if (Text::endsWith($uri, 'servicepackdeploy') || Text::endsWith($uri, 'upgradedeploy')) {
...
$GLOBALS['EXT'] = $EXT =& load_class('Hooks');
...
$GLOBALS['CFG']  = $CFG  =& load_class('Config');
$GLOBALS['RTR']  = $RTR  =& load_class('Router');
$GLOBALS['OUT']  = $OUT  =& load_class('Output');
$GLOBALS['IN']   = $IN   =& load_class('Input');
$GLOBALS['URI']  = $URI  =& load_class('URI');
