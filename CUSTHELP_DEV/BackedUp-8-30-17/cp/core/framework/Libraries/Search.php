        ...
        if (!$this->getErrors()) {
            ...
            $this->callOnModelsForSources($this->getSources(), function ($model, $sourceID) use (&$allResults, $filters, &$cache) {
                ...
                if ($results instanceof SearchResults) {
                    ...
                    return Config::ASTRgetMessage("The results returned from the model was not a \RightNow\Libraries\SearchResults object.");
                }
            });
    ...
    private function callOnModelsForSources (array $sources, \Closure $callback) {
        foreach ($sources as $sourceID => $source) {
            ...
            if (!$model instanceof SearchSourceBase) {
                return $this->addError(sprintf(Config::ASTRgetMessage("The '%s' model designated by the %s search source must extend from `\RightNow\Models\SearchSourceBase`"), $source['model'], $sourceID));
            }
