...
abstract class Base{
    ...
    private static function getExtendedModel($model){
        ...
        if(self::$modelExtensionList === null){
            $extensionsList = @yaml_parse_file(APPPATH . "config/extensions.yml");
            if(is_array($extensionsList) && array_key_exists('modelExtensions', $extensionsList)){
                if(is_array($extensionsList['modelExtensions'])){
                    self::$modelExtensionList = $extensionsList['modelExtensions'];
                }
                else{
