...
class Social extends AsyncBase
{
     ...
     * @param boolean $strictMode Denotes if validation should occur when building up the SSO token.
     ...
     */
    public function generateSsoToken($strictMode = false, $redirect = '')
    {
        if(!Framework::isLoggedIn())
        ...
        foreach($this->getSignatureParameters($profileData) as $key => $detail)
        {
            if($strictMode && ($detail === null || $detail === '' || $detail === false))
                $this->getResponseObject(null, 'is_null', "Validation of token parameters failed. The '$key' parameter does not have a value.");
            $encodedSsoDetails .= sprintf('%s%s=%s', $encodedSsoDetails ? '&' : '', $key, urlencode($detail));
        }
        return $this->getResponseObject(strtr(base64_encode($encodedSsoDetails), array('+' => '_', '/' => '~', '=' => '*')), 'is_string');
    }
     ...
     */
    private function getSignatureParameters(array $profileData)
    {
        ...
        $parameters['Signature'] = $this->hexToBase64($this->calculateApiSignature($parameters));
        $parameters['p_sessionid'] = $this->CI->session->getSessionData('sessionID');
        $parameters['p_timestamp'] = time();

        if($redirect = $profileData['redirectUrl']) {
            $parameters['redirectUrl'] = $redirect;
        }
    ...
    private function generateCommunityApiUrl(array $signatureParameters, $restEndpoint = null)
    {
        ...
        $signature = $this->calculateApiSignature($signatureParameters);
    ...
    private function processCommunityResponse($result, $subObject, Libraries\ResponseObject $responseObject, $callback) {
        if ($result) {
            ...
            if ($object === null) {
                ...
                $responseObject->error = new \RightNow\Libraries\ResponseError(Config::getMessage(ERROR_REQUEST_ACTION_COMPLETED_MSG));
            }
            else if ($object->error) {
                // Community responded with an error
                Api::phpoutlog("Received error code {$object->error->code} from the community API");
                $responseObject->error = new \RightNow\Libraries\ResponseError(Config::getMessage(ERROR_REQUEST_ACTION_COMPLETED_MSG), $object->error->code);
            }
            else if (!$subObject) {
     ...
     * @return string Encoded signature parameter
     */
    private function calculateApiSignature($signatureParameters)
    {
        $stringToSign = '';
        foreach($signatureParameters as $key => $val)
