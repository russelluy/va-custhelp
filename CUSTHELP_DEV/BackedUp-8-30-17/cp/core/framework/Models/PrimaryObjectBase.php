...
abstract class PrimaryObjectBase extends Base{
    ...
    function __construct($objectName)
    {
        ...
        if(!class_exists($namespacedClass)){
            throw new \Exception("Class $namespacedClass does not exist.");
        }
     ...
     * For CX versions 16.5 and above we will use the API function fattach_full_path in place of the config setting upload_tmp_dir.
    ...
    protected function createAttachmentEntry(Connect\RNObject $connectObject, $value){
        if(is_array($value) && count($value)){
            ...
            foreach($value as $attachment){
                if(($tempFile = (\RightNow\Internal\Utils\Version::compareVersionNumbers(\RightNow\Internal\Utils\Version::getVersionNumber(MOD_BUILD_VER), "16.5") >= 0) ? \RightNow\Api::fattach_full_path($attachment->localName) : get_cfg_var('upload_tmp_dir') . '/' . $attachment->localName) && \RightNow\Utils\FileSystem::isReadableFile($tempFile)){
    ...
    protected function getSaveErrors(\Exception $exception) {
        ...
        if($this->objectName === 'Contact' && \RightNow\Utils\Text::stringContains($message, 'Contact.NewPassword')) {
