...
RightNow.Chat.UI.EventBus = function()
{
    ...
    function onChatDisconnectResponse(type, eventData)
    {
        ...
        if(reason === reasons.PARTICIPANT_LEFT || reason === reasons.TRANSFERRED_TO_QUEUE)
...
Y.on('domready', function(){
   ...
   if(Y.UA.safari){
    ...
    //Prevent end user from using F5 in the chat window (as this will terminate the chat - see onWindowUnload)
    //You only want to disconnect when either pressing 'Disconnect' or by closing the popup.
    Y.on("key", function(e){
        e.preventDefault();
    }, Y.one(document), "down:116");
    Y.on("key", function(e){
        e.preventDefault();
    }, Y.one(document), "down:82+ctrl");
}, this);
});
