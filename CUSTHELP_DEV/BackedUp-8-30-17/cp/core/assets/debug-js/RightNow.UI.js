...
RightNow.UI.Dialog = (function()
{
    ...
    return {
        ...
        */
        ...
        /**
         * Due to an unusual YUI bug, panels nested within dialogs inadvertently hide the parent dialog's mask
         * when hidden. Add this handler to panels to prevent that behavior.
         * @param  {Object} panel YUI Panel instance
         */
        remaskDialogOnHide: function(panel) {
            panel.after('visibleChange', function(e) {
                var mask = Y.one('.yui3-widget-mask');
                if(!e.newVal && panel.get('boundingBox').ancestor('.yui3-panel') && mask) {
                    mask.setStyle('display', 'block').removeAttribute('hidden');
                }
            });
        }
    };
