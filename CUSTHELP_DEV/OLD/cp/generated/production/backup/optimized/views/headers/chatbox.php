...
get_instance()->_checkMeta(array('title'=>'' . \RightNow\Utils\Config::msgGetFrom((4525)) . '', 'template'=>'standard_noatg.php', 'clickstream'=>'home'));
...
function _standard_chat_ConditionalChatLink_view ($data) {
extract($data);
...
function getData() {
$this->data['js']['dqaWidgetType'] = (6);
$this->data['js']['dqaInsertType'] = (9);
$this->data['js']['avail_type'] = "sessions";
...
if($organizationID > 0) $this->data['js']['org_id'] = $organizationID;
...
if(!$this->data['attrs']['initiate_by_event'] && $this->data['attrs']['enable_availability_check']) $this->processDataFromCheckChatQueue($linkUrl);
if($this->data['attrs']['auto_detect_incident'] && $incidentID = Url::getParameter('i_id')) $linkUrl .= "/i_id/$incidentID";
$linkUrl .= '/request_source/' . (7);
...
if(!$prodValue && $answer->Products && ($prodValue = $this->CI->model('Answer')->getFirstBottomMostProduct($answerID)->result)) $prodValue = $prodValue['ID'];
if(!$catValue && $answer->Categories && ($catValue = $this->CI->model('Answer')->getFirstBottomMostCategory($answerID)->result)) $catValue = $catValue['ID'];
}
}
else if($incidentID = Url::getParameter('i_id')) {
if($incident = $this->CI->model('Incident')->get($incidentID)->result) {
if(!$prodValue && $incident->Product) $prodValue = $incident->Product->ID;
if(!$catValue && $incident->Category) $catValue = $incident->Category->ID;
}
}
}
...
$chatQueueResult = $this->CI->model('Chat')->checkChatQueue($chatRouteRV, "sessions", true)->result;
...
if($surveyData['comp_id']) $linkUrl .= '/survey_comp_id/' . $surveyData['comp_id'];
if($surveyData['comp_auth']) $linkUrl .= '/survey_comp_auth/' . $surveyData['comp_auth'];
if($surveyData['term_id']) $linkUrl .= '/survey_term_id/' . $surveyData['term_id'];
if($surveyData['term_auth']) $linkUrl .= '/survey_term_auth/' . $surveyData['term_auth'];
if($surveyData['send_id']) {
$linkUrl .= '/survey_send_id/' . $surveyData['send_id'];
$linkUrl .= '/survey_send_delay/' . $surveyData['send_delay'];
}
if($surveyData['send_auth']) $linkUrl .= '/survey_send_auth/' . $surveyData['send_auth'];
}
if(!$this->data['attrs']['ignore_preroute']) {
$routeData = '';
if(isset($chatQueueResult['q_id']) && $chatQueueResult['q_id'] > 0) $routeData .= 'q_id=' . $chatQueueResult['q_id'] . '&';
if(isset($chatQueueResult['rules'])) {
$ruleData = $chatQueueResult['rules'];
if($ruleData['state']) $routeData .= 'state=' . $ruleData['state'] . '&';
if($ruleData['escalation']) $routeData .= 'escalation=' . $ruleData['escalation'];
}
if(strlen($routeData) !== 0) $linkUrl .= '/chat_data/' . base64_encode($routeData);
}
}
}
function _standard_chat_ConditionalChatLink_header() {
$result = array( 'js_name' => 'RightNow.Widgets.ConditionalChatLink', 'library_name' => 'ConditionalChatLink', 'view_func_name' => '_standard_chat_ConditionalChatLink_view', 'meta' => array ( 'controller_path' => 'standard/chat/ConditionalChatLink', 'view_path' => 'standard/chat/ConditionalChatLink', 'js_path' => 'standard/chat/ConditionalChatLink', 'js_templates' => array ( 'availableImmediatelyMessage' => '<div class="rn_Chat">  <a href="javascript:void(0);" id="rn_<%= instanceID %>_AvailableImmediatelyLink"><%= linkTitle %></a>   <span id="rn_<%= instanceID %>_AvailableImmediatelyMessage"><%= message %></span> </div>', 'availableWithWaitMessage' => '<div class="rn_Chat">  <a href="javascript:void(0);" id="rn_<%= instanceID %>_AvailableWithWaitLink"><%= linkTitle %></a>   <span id="rn_<%= instanceID %>_AvailableWithWaitMessage"><%= message %></span> </div>', 'defaultMessage' => '<div class="rn_Chat">  <a href="javascript:void(0);" id="rn_<%= instanceID %>_DefaultLink"><%= linkTitle %></a>   <span id="rn_<%= instanceID %>_DefaultMessage"><%= message %></span> </div>', 'unavailableBusyMessage' => '<div class="rn_Chat">  <a href="javascript:void(0);"><%= linkTitle %></a>   <span id="rn_<%= instanceID %>_UnavailableBusyMessage"><%= message %></span> </div>', 'unavailableHoursMessage' => '<div class="rn_Chat">  <a href="javascript:void(0);"><%= linkTitle %></a>   <span id="rn_<%= instanceID %>_UnavailableHoursMessage"><%= message %></span> </div>', ), 'template_path' => 'standard/chat/ConditionalChatLink', 'version' => '1.2.1', 'requires' => array ( 'framework' => array ( 0 => '3.2', ), 'jsModule' => array ( 0 => 'standard', 1 => 'mobile', ), ), 'info' => array ( 'description' => 'rn:msg:(4326)', 'urlParameters' => array ( 'a_id' => array ( 'name' => 'rn:msg:(4653)', 'description' => 'rn:msg:(999)', 'example' => 'a_id/3', ), 'i_id' => array ( 'name' => 'rn:msg:(10071)', 'description' => 'rn:msg:(2168)', 'example' => 'i_id/7', ), 'p' => array ( 'name' => 'rn:msg:(4623)', 'description' => 'rn:msg:(3276)', 'example' => 'p/7,2', ), 'c' => array ( 'name' => 'rn:msg:(4617)', 'description' => 'rn:msg:(1122)', 'example' => 'c/8,3', ), ), ), 'relativePath' => 'standard/chat/ConditionalChatLink', 'widget_name' => 'ConditionalChatLink', ), );
$result['meta']['attributes'] = array( 'get_chat_info_ajax' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => '//' . \RightNow\Utils\Config::getConfig((56), 'common') . '/ci/ajaxRequest/getChatQueueAndInformation', 'type' => 'AJAX', 'default' => '//' . \RightNow\Utils\Config::getConfig((56), 'common') . '/ci/ajaxRequest/getChatQueueAndInformation', 'inherited' => false, )), 'wait_threshold' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => 40, 'type' => 'INT', 'default' => 40, 'inherited' => false, )), 'min_sessions_avail' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => 0, 'type' => 'INT', 'default' => 0, 'inherited' => false, )), 'chat_login_page' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => '/app/' . \RightNow\Utils\Config::getConfig((223), 'RNW_UI'), 'type' => 'STRING', 'default' => '/app/' . \RightNow\Utils\Config::getConfig((223), 'RNW_UI'), 'inherited' => false, )), 'chat_login_page_width' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => 700, 'type' => 'INT', 'default' => 700, 'inherited' => false, )), 'chat_login_page_height' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => 686, 'type' => 'INT', 'default' => 686, 'inherited' => false, )), 'open_in_new_window' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => false, 'type' => 'BOOL', 'default' => false, 'inherited' => false, )), 'auto_detect_incident' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => false, 'type' => 'BOOL', 'default' => false, 'inherited' => false, )), 'enable_availability_check' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => true, 'type' => 'BOOL', 'default' => true, 'inherited' => false, )), 'label_available_with_wait_template' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(39855), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(39855), 'inherited' => false, )), 'label_available_immediately_template' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(39856), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(39856), 'inherited' => false, )), 'label_unavailable_busy_template' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(39854), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(39854), 'inherited' => false, )), 'label_unavailable_hours' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(39886), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(39886), 'inherited' => false, )), 'hide_on_unavailable' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => false, 'type' => 'BOOL', 'default' => false, 'inherited' => false, )), 'label_default' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => \RightNow\Utils\Config::getMessage(1149), 'type' => 'STRING', 'default' => \RightNow\Utils\Config::getMessage(1149), 'inherited' => false, )), 'enable_polling' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => true, 'type' => 'BOOL', 'default' => true, 'inherited' => false, )), 'ignore_preroute' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => false, 'type' => 'BOOL', 'default' => false, 'inherited' => false, )), 'initiate_by_event' => \RightNow\Libraries\Widget\Attribute::__set_state(array( 'value' => false, 'type' => 'BOOL', 'default' => false, 'inherited' => false, )), );
return $result;
}
}
namespace RightNow\Widgets{
...
<link href='<?=FileSystem::getOptimizedAssetsDir();?>templates/standard_noatg.themes.standard.SITE.css' rel='stylesheet' type='text/css' media='all'/>
<style type="text/css">
<!--
...
<!--// START - This code needs to go on VA site -->
<div id="c2c-fare-selector">
    <div id="chat-reactive-chat-link">
	 <div id="myChatLinkInfo">
     </div>
    </div>
</div>
<?=\RightNow\Utils\Widgets::rnWidgetRenderCall('standard/chat/ConditionalChatLink',
array('min_sessions_avail' => '1',));
?>
...
<!--// END - This code needs to go on VA site ?>-->
...
( 0 => 'standard/login/LogoutLink', 1 => 'standard/login/LoginDialog', ), '/standard_noatg.js', '1436303720');?>"></script>
