...
<? if($this->field->data_type === EUF_DT_SELECT || $this->field->data_type === EUF_DT_CHECK):?>
    ...
    <? if(is_array($this->data['menuItems'])):?>
        <? foreach($this->data['menuItems'] as $key => $item):
            $selected = ((string)$key == $this->data['value']) ? 'selected="selected"' : '';?>
...
<? elseif($this->field->data_type === EUF_DT_RADIO):?>
    <?if($this->data['attrs']['display_as_checkbox']):
        $checked = ($this->data['checkedIndex'] === 1) ? "checked='checked'" : '';?>
        <label for="rn_<?=$this->instanceID;?>_<?=$this->data['js']['name'];?>" id="rn_<?=$this->instanceID;?>_Label" class="rn_Label"><?=$this->data['attrs']['label_input'];?>
        <? if($this->data['attrs']['required']):?>
            <span class="rn_Required"> <?=getMessage(FIELD_REQUIRED_MARK_LBL);?> </span><span class="rn_ScreenReaderOnly"><?=getMessage(REQUIRED_LBL)?></span>
        <? endif;?>
    ...
    <?else:?>
        ...
        <? for($i = 1; $i >= 0; $i--):
            ...
            <label for="<?=$id;?>"><?=$this->data['radioLabel'][$i];?><? if($this->data['js']['hint'] && !$this->data['attrs']['hide_hint'] && $i===1):?> <span class="rn_ScreenReaderOnly"><?=$this->data['js']['hint']?></span><?endif;?></label>
        <? endfor;?>
