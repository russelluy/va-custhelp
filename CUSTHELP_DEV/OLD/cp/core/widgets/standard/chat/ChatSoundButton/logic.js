...
RightNow.Widgets.ChatSoundButton = RightNow.Widgets.extend({
    constructor: function(){
        ...
        {
            if(this.data.attrs.receive_sound_ogg_path && new Audio().canPlayType('audio/ogg').match(html5Test))
            {
                oggEnabled = true;
            }
            else if(this.data.attrs.receive_sound_wav_path && new Audio().canPlayType('audio/x-wav').match(html5Test))
            {
                wavEnabled = true
            }
        }
